...
def _save_new_habit(uid, u):
    ...
    if hab_type == "repeat":
        ...
        rep_count   = temp.get("repeat_count", len(times_list))
    ...
    if hab_type == "repeat":
        valid_times = [t for t in new_habit.get("repeat_times", [])
                       if t and t not in ("vaqtsiz", "—", "", None)]
        if valid_times:
    ...
    # Foydalanuvchiga ko'rsatiladigan vaqt
    if hab_type == "repeat":
        valid_times = [t for t in new_habit.get("repeat_times", [])
                       if t and t not in ("vaqtsiz", "—", "", None)]
        time_show = ", ".join(valid_times) if valid_times else "—"
    else:
