#!/usr/bin/env python3
"""
Komandalar: /start, /admin_panel, /test_onboard, contact handler
"""
...
from bot_setup import (bot, send_main_menu, main_menu, main_menu_dict,
                       send_message_colored, cBtn, ok_kb, get_bot_username)
from menus import (check_subscription, send_sub_required, admin_menu)
...
def cmd_start(msg):
    ...
    # ── BARCHA OLDINGI XABARLARNI O'CHIRISH (48 soat ichida) ──
    # /start msg_id dan 1 gacha pastga qarab hammasini o'chirishga urinadi
    current_msg_id = msg.message_id
    def clear_all_messages(chat_id, up_to_msg_id):
        for mid in range(up_to_msg_id - 1, max(up_to_msg_id - 500, 0), -1):
            try:
                bot.delete_message(chat_id, mid)
            except Exception:
                pass
    threading.Thread(
        target=clear_all_messages,
        args=(uid, current_msg_id),
        daemon=True
    ).start()
    ...
    u_prev.pop("lang_msg_id", None)
    u_prev.pop("reg_msg_id", None)
    u_prev.pop("sub_msg_id", None)
    u_prev.pop("start_cmd_msg_id", None)
    u_prev.pop("main_msg_id", None)
