...
function renderToday(d) {
  ...
  let cardsHtml = '';
  // Ajratuvchi chiziq uchun: oxirgi ko'rilgan PENDING odatning darajasi.
  // Done odatlar bu kuzatuvga ta'sir qilmaydi — shuning uchun done odatlar
  // yuqoriga uyilganda ham chiziq osilib qolmaydi.
  let _lastPendingPrio = null;
  sortedHabits.forEach((h) => {
    // ── AJRATUVCHI CHIZIQ ──
    // Faqat pending (bajarilmagan) odatlar orasida, muhimlik darajasi
    // o'zgargan joyda 1 ta chiziq (high|medium va medium|low → ko'pi 2 ta).
    // Done bo'lgan odat _lastPendingPrio ni o'zgartirmaydi va chiziq olmaydi.
    if (!h.done) {
      const curPrio = _prioOf(h);
      if (_lastPendingPrio !== null && _lastPendingPrio !== curPrio) {
        cardsHtml += '<div class="prio-divider"></div>';
      }
      _lastPendingPrio = curPrio;
    }
    const rc  = h.repeat_count || 1;
    const tc  = h.today_count  || 0;
    const isRepeat = rc > 1;
    // Pending repeat habit'da `2/3` koʻrinadigan label (oddiy/done holatda boʻsh — SVG ichida ✓)
    const btnContent = h.done ? '' : (isRepeat && tc > 0 ? tc+'/'+rc : '');
    // Halqa progress foizi: oddiy habit pending=0%, done=100%; repeat habit tc/rc nisbatida
    const ringPct = h.done ? 100 : (isRepeat ? Math.round(tc / rc * 100) : 0);
      ...
      <div class="checkin-card ${h.done ? 'done' : ''} ${d.is_today_view === false ? 'readonly' : ''}" id="ccard-${h.id}">
        ...
        <div class="checkin-front" data-hid="${h.id}" onclick="checkinFromFront('${h.id}', this)">
          ...
          <div class="checkin-info">
            ...
            <div class="checkin-meta">${isRepeat ? rc+S('today','times_per_day') : (h.time !== 'vaqtsiz' ? `<svg width="13" height="13" viewBox="0 0 20 20" fill="none" style="display:inline;vertical-align:middle"><defs><linearGradient id="svgClock" x1="0" y1="0" x2="20" y2="20" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="#A8ADB5"/><stop offset="100%" stop-color="#8A8F98"/></linearGradient></defs><circle cx="10" cy="10" r="8" stroke="url(#svgClock)" stroke-width="2"/><path d="M10 6V10L13 12" stroke="url(#svgClock)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg> `+h.time : '')}${h.streak > 0 ? ` · <svg width="13" height="13" viewBox="0 0 20 20" fill="none" style="display:inline;vertical-align:middle"><defs><linearGradient id="svgFire" x1="10" y1="0" x2="10" y2="20" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="#A8ADB5"/><stop offset="100%" stop-color="#8A8F98"/></linearGradient></defs><path d="M10 2C10 2 14 6 14 10C14 12 13 13.5 11.5 14.5C12 13 11.5 11.5 10.5 11C11 13 9.5 15 8 15.5C9 14 8.5 12 7 11C5.5 12.5 6 15 7 16.5C5.5 15.5 4 13.5 4 11C4 7 8 4 10 2Z" fill="url(#svgFire)"/></svg> ${h.streak}` : ''}</div>
            ${dotsHtml}
          </div>
          ...
          <button class="checkin-btn" id="cbtn-${h.id}" onclick="event.stopPropagation();checkin('${h.id}', document.getElementById('ccard-${h.id}'))">${checkinRingHTML(ringPct, h.done, btnContent)}</button>
...
async function checkin(hid, cardEl) {
  ...
  try {
    ...
    if (btn) {
      const isRepeat = rc > 1;
      const lbl = isDone ? '' : (isRepeat && tc > 0 ? tc+'/'+rc : '');
      const ringPct = isDone ? 100 : (isRepeat ? Math.round(tc / rc * 100) : 0);
      btn.innerHTML = checkinRingHTML(ringPct, isDone, lbl);
      btn.style.fontSize = '';
    }
    ...
    if (metaEl) {
      const isRepeat = rc > 1;
      const habitForMeta = (data.today && data.today.habits || []).find(x => x.id === hid);
      const habitTime = habitForMeta ? habitForMeta.time : 'vaqtsiz';
      metaEl.innerHTML = isRepeat ? `${rc}${S('today','times_per_day')}${result.streak > 0 ? ` · <svg width="13" height="13" viewBox="0 0 20 20" fill="none" style="display:inline;vertical-align:middle"><defs><linearGradient id="svgFire" x1="10" y1="0" x2="10" y2="20" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="#A8ADB5"/><stop offset="100%" stop-color="#8A8F98"/></linearGradient></defs><path d="M10 2C10 2 14 6 14 10C14 12 13 13.5 11.5 14.5C12 13 11.5 11.5 10.5 11C11 13 9.5 15 8 15.5C9 14 8.5 12 7 11C5.5 12.5 6 15 7 16.5C5.5 15.5 4 13.5 4 11C4 7 8 4 10 2Z" fill="url(#svgFire)"/></svg> ${result.streak}` : ''}` : `${habitTime !== 'vaqtsiz' ? `<svg width="13" height="13" viewBox="0 0 20 20" fill="none" style="display:inline;vertical-align:middle"><defs><linearGradient id="svgClock" x1="0" y1="0" x2="20" y2="20" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="#A8ADB5"/><stop offset="100%" stop-color="#8A8F98"/></linearGradient></defs><circle cx="10" cy="10" r="8" stroke="url(#svgClock)" stroke-width="2"/><path d="M10 6V10L13 12" stroke="url(#svgClock)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg> ${habitTime}` : ''}${result.streak > 0 ? `${habitTime !== 'vaqtsiz' ? ' · ' : ''}<svg width="13" height="13" viewBox="0 0 20 20" fill="none" style="display:inline;vertical-align:middle"><defs><linearGradient id="svgFire" x1="10" y1="0" x2="10" y2="20" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="#A8ADB5"/><stop offset="100%" stop-color="#8A8F98"/></linearGradient></defs><path d="M10 2C10 2 14 6 14 10C14 12 13 13.5 11.5 14.5C12 13 11.5 11.5 10.5 11C11 13 9.5 15 8 15.5C9 14 8.5 12 7 11C5.5 12.5 6 15 7 16.5C5.5 15.5 4 13.5 4 11C4 7 8 4 10 2Z" fill="url(#svgFire)"/></svg> ${result.streak}` : ''}`;
    }
