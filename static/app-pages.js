      ...
      <div class="checkin-card ${h.done ? 'done' : ''}" id="ccard-${h.id}">
        ...
        <div class="checkin-front" data-hid="${h.id}" onclick="checkinFromFront('${h.id}', this)">
          ...
          <div class="checkin-info">
            ...
            <div class="checkin-meta">${isRepeat ? rc+S('today','times_per_day')+' · ' : (h.time !== 'vaqtsiz' ? `<svg width="13" height="13" viewBox="0 0 20 20" fill="none" style="display:inline;vertical-align:middle"><defs><linearGradient id="svgClock" x1="0" y1="0" x2="20" y2="20" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="#5B8DEF"/><stop offset="100%" stop-color="#A78BFA"/></linearGradient></defs><circle cx="10" cy="10" r="8" stroke="url(#svgClock)" stroke-width="2"/><path d="M10 6V10L13 12" stroke="url(#svgClock)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg> `+h.time+' · ' : '')}<svg width="13" height="13" viewBox="0 0 20 20" fill="none" style="display:inline;vertical-align:middle;opacity:${h.streak > 0 ? '1' : '0.5'}"><defs><linearGradient id="svgFire" x1="10" y1="0" x2="10" y2="20" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="${h.streak > 0 ? '#F6C93E' : '#C5C9D1'}"/><stop offset="100%" stop-color="${h.streak > 0 ? '#E07040' : '#9BA0AA'}"/></linearGradient></defs><path d="M10 2C10 2 14 6 14 10C14 12 13 13.5 11.5 14.5C12 13 11.5 11.5 10.5 11C11 13 9.5 15 8 15.5C9 14 8.5 12 7 11C5.5 12.5 6 15 7 16.5C5.5 15.5 4 13.5 4 11C4 7 8 4 10 2Z" fill="url(#svgFire)"/></svg> ${h.streak}</div>
            ${dotsHtml}
            ${(() => {
