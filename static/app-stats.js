...
function renderRating(d) {
  ...
  // Inventory cache — har render da tozalanadi, click handler'lar index orqali oladi
  window._invCache = {};
  ...
  // Inventory cache key yaratish (u.uid yoki auto key) va global state'ga saqlash
  function _invKey(u) {
    const key = 'u' + (u.uid || Math.random().toString(36).slice(2));
    window._invCache[key] = {
      name: u.name || '',
      list: u.items_list || []
    };
    return key;
  }
  // Banda ichidagi "trofey ko'rgazmasi" — eng qimmat top-1 emoji + qolgani "+N"
  // Sabab: 🎒 N quruq band edi, foydalanuvchi maqtana olmasdi. Endi eng qimmat
  // buyum emojisi to'g'ridan-to'g'ri ko'rinadi (passiv maqtanish).
  ...
  function _invBadgeDisplay(u) {
    ...
    // Eng qimmat buyum oldinda (price kamayuvchi) — faqat top-1 ko'rinadi, qolgani +N
    ...
    podiumHtml += `
      <div style="flex:1;max-width:110px;text-align:center">
        ...
        <div style="height:${h}px;background:linear-gradient(180deg,${col}44,${col}22);
          ...
          display:flex;flex-direction:column;align-items:center;justify-content:flex-end;padding-bottom:21px;gap:3px">
    ...
    rowsHtml += `
      <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;
        ...
        <div style="flex:1;min-width:0">
          <div style="font-size:13px;font-weight:700;color:${isMe?'var(--accent)':'var(--text)'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${u.name}${u.badge?' '+u.badge:''}</div>
        </div>
  ...
  if (caller_entry) {
    ...
    myRowHtml = `
      <div style="margin-top:12px;border-top:1px solid rgba(120,120,128,0.2);padding-top:12px">
        ...
        <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;
          ...
          ${userAvatarHTML(u, 34)}
          <div style="flex:1;min-width:0">
            <div style="font-size:13px;font-weight:700;color:var(--accent);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${u.name}</div>
          </div>
          <div style="display:flex;align-items:center;gap:5px;flex-shrink:0">
            ${u.items_count > 0 ? `<div class="inv-badge-clickable" onclick="event.stopPropagation();openUserInventoryByKey('${_invKey(u)}')" style="flex-shrink:0;font-size:11px;font-weight:700;color:var(--accent);background:rgba(76,175,125,0.13);border-radius:6px;padding:2px 5px;white-space:nowrap;cursor:pointer;letter-spacing:-0.5px">${_invBadgeDisplay(u)}</div>` : ''}
            ${u.habits_count ? `<div style="flex-shrink:0;font-size:9px;font-weight:700;color:#4CAF7D;background:#4CAF7D18;border-radius:6px;padding:2px 5px;white-space:nowrap;min-width:${_habW};display:inline-flex;align-items:center;justify-content:flex-end;gap:3px"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" style="display:inline;vertical-align:middle;flex-shrink:0"><defs><linearGradient id=\"svgClipRat\" x1=\"0\" y1=\"0\" x2=\"24\" y2=\"24\" gradientUnits=\"userSpaceOnUse\"><stop offset=\"0%\" stop-color=\"#2D8A5E\"/><stop offset=\"100%\" stop-color=\"#4CAF7D\"/></linearGradient></defs><rect x=\"8\" y=\"2\" width=\"8\" height=\"4\" rx=\"1\" fill=\"url(#svgClipRat)\"/><rect x=\"4\" y=\"4\" width=\"16\" height=\"18\" rx=\"2\" fill=\"url(#svgClipRat)\" opacity=\"0.15\"/><rect x=\"4\" y=\"4\" width=\"16\" height=\"18\" rx=\"2\" stroke=\"url(#svgClipRat)\" stroke-width=\"1.5\"/><path d=\"M8 11h8M8 15h5\" stroke=\"url(#svgClipRat)\" stroke-width=\"1.5\" stroke-linecap=\"round\"/></svg> ${u.habits_count}</div>` : ''}
...
// Cache orqali chaqirish (reyting sahifasida ishlatiladi — HTML attribute escape muammolarini oldini oladi)
function openUserInventoryByKey(key) {
  const cache = window._invCache || {};
  const data = cache[key];
  if (!data) {
    console.warn('Inventory cache key not found:', key);
    return;
  }
  openUserInventory(data.name, data.list);
}
...
function openUserInventory(userName, itemsList) {
  ...
  const closeLbl = _invEscapeHtml(S('inventory','modal_close'));

  let itemsHtml = '';
  if (!itemsList || itemsList.length === 0) {
    itemsHtml = `<div style="text-align:center;color:var(--sub);padding:20px 0;font-size:14px">${_invEscapeHtml(S('inventory','modal_empty'))}</div>`;
  } else {
    ...
    itemsList.forEach(it => {
      ...
      if (it.qty > 1) {
        if (it.id === 'xp_booster') {
          qtyText = ` <span style="color:var(--sub);font-weight:500">· ${it.qty} ${_invEscapeHtml(S('inventory','modal_days'))}</span>`;
        } else {
...
async function openUserProfile(targetUid) {
  ...
  loadingModal.innerHTML = `
    <div class="shop-modal-box" onclick="event.stopPropagation()" style="max-width:480px;width:calc(100% - 24px);padding:0;overflow:visible;position:relative;text-align:left">
