...
| `style.css` | 1128 | Barcha CSS: `:root` ranglar, dark mode, neumorphism shadow'lar, `.header` da `--tg-safe-top` safe area padding, card/button/modal/toast/nav stillar, animatsiyalar (spin, fadeIn, slideUp, confetti), PTR indikator, nav ball glow pulsatsiya (`ballGlow`), toʻlqin ripple effekti (`navRippleWave`) |
