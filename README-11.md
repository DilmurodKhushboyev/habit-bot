...
| `flask_routes_extra.py` | ~806 | `/api/achievements`, `/api/shop`, `/api/friends`, `/api/challenges`, `/api/reminder` (repeat\_times, reminder\_enabled, repeat qo'llab-quvvatlaydi), `/health`, webhook. **Import tuzatish:** `telebot`, `InlineKeyboardMarkup/Button`, `ACHIEVEMENTS as _ACHIEVEMENTS`, `CAT_LABELS` qo'shildi |
...
| `style.css` | 1049 | Barcha CSS: `:root` ranglar, dark mode, neumorphism shadow'lar, card/button/modal/toast/nav stillar, animatsiyalar (spin, fadeIn, slideUp, confetti), PTR indikator, nav ball glow pulsatsiya (`ballGlow`), toʻlqin ripple effekti (`navRippleWave`) |
| `strings.js` | 885 | `STRINGS` obyekt (UZ/RU/EN tarjimalar), `S(key, sub)` funksiya, `selectedLang`, `currentLang` o'zgaruvchilari |
| `app-core.js` | 236 | Telegram WebApp init (`tg.ready()`, `tg.expand()`), `user`/`userId`/`API` konstantalari, `data`/`loaded` state, `switchTab()`/`goBack()`/`loadTab()`, `apiFetch()` (20s timeout), `showPremiumPage()`/`renderPremium()`/`buyPremium()`, `ringHTML()`/`jonRingHTML()` SVG generatorlar, `spawnNavRipple()` toʻlqin effekti |
