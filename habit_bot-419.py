...
try:
    ...
    def api_share_card(uid):
        ...
        try:
            ...
            caption_text = "📊 " + T(uid, "weekly_share_caption")
            share_text = caption_text + "\n\nhttps://t.me/Super_habits_bot"
            share_url = "https://t.me/share/url?url=" + urllib.parse.quote("https://t.me/Super_habits_bot") + "&text=" + urllib.parse.quote(share_text)
            kb = InlineKeyboardMarkup()
            kb.row(
                InlineKeyboardButton(T(uid, "share_fwd_btn"), url=share_url),
                InlineKeyboardButton(T(uid, "share_del_btn"), callback_data="share_del")
            )
            bot.send_photo(uid, photo_bytes, caption=caption_text, reply_markup=kb)
            return jsonify({"ok": True})
        except Exception as e:
