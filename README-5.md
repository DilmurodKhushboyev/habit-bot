...
6. **Frontend cache-busting**: barcha `<script src>` va `<link href>` larda `?v=418` query string bor — yangilanganda versiyani oshiring
...
9. **Frontend rang sxemasi (v418)**: Emerald/teal tema. CSS variables `style.css` `:root` da. Rang xaritasi:
   - `--accent: #10B981` (emerald) — asosiy aksent
   - `--accent2: #059669` (teal) — ikkinchi aksent
   - `--green: #10B981` — ijobiy natija
   - `--warn: #F59E0B` — ogohlantirish/pasayish (sariq)
   - `--red: #EF4444` — yomon natija
   - `--gold: #D4963A` — ball/medal (saqlanib qoldi)
   - SVG gradient ranglar: `#34D399` (och emerald), `#059669` (teal), `#047857` (quyuq)
   - Warning SVG: `#FBBF24` → `#F59E0B` (sariq gradient)
   - Shadow bazaviy rang: `#B4BDB8` (yashil-kulrang)
