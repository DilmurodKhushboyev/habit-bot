...
├── callbacks_menu.py         ← Menyu navigatsiya, hisobot ro'yxati, onboard
...
| `handlers_onboarding.py` | 182 | Yangi foydalanuvchi uchun: kategoriya tanlash → odat tanlash → vaqt tanlash → saqlash |
| `handlers_callbacks.py` | 145 | **Dispatcher**: umumiy preamble (til, obuna) + 6 ta sub-handlerga yo'naltirish |
| `handlers_text.py` | 1055 | Barcha matn xabarlari (state machine), Telegram Stars to'lov, broadcast, inline query |
...
| `callbacks_menu.py` | 415 | Menyu navigatsiya, hisobot ro'yxatlari (weekly/monthly/yearly view), onboard callbacklar |
...
| `flask_routes_core.py` | 508 | `/api/rating`, `/api/profile`, `/api/habits` CRUD, `/api/groups` CRUD |
| `flask_routes_data.py` | 485 | `/api/today`, `/api/checkin` (bajarish logikasi), `/api/stats` |
| `flask_routes_extra.py` | 774 | `/api/achievements`, `/api/shop`, `/api/friends`, `/api/challenges`, `/health`, webhook |
