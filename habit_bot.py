...
def handle_text(msg):
    ...
    if state == "renaming_habit":
        ...
        # settings_info ga qaytish
        kb_info = InlineKeyboardMarkup()
        kb_info.add(cBtn("✏️ Ismni o'zgartirish",            "change_name",        "primary"))
        kb_info.add(cBtn("📱 Telefon raqamni o'zgartirish",  "change_phone",       "primary"))
        kb_info.add(cBtn("✏️ Odat nomini o'zgartirish",      "change_habit_name",  "primary"))
        kb_info.row(
    ...
    if state == "updating_name":
        ...
        # Sozlamalar menyusiga qaytish
        kb_set = InlineKeyboardMarkup()
        kb_set.add(cBtn(T(uid, "btn_change_lang"),         "settings_lang", "primary"))
        kb_set.add(cBtn(T(uid, "btn_change_info"),         "settings_info", "primary"))
        kb_set.add(cBtn("⏰ Odatlar vaqtini tahrirlash",   "settings_habits_time", "primary"))
        kb_set.add(cBtn("💬 Dasturchiga habar",            "settings_contact_dev", "primary"))
...
def delete_habit_menu(uid):
    ...
    kb = InlineKeyboardMarkup()
    for h in sorted_habits:
        kb.add(cBtn(f"🗑 {h['name']} ({h['time']})", f"delete_{h['id']}", "danger"))
    kb.row(
        cBtn(T(uid, "btn_home"), "menu_main", "primary")
    )
...
def callback_handler(call):
    ...
    if cdata == "change_habit_name":
        ...
        for h in sorted_h:
            kb_hn.add(cBtn(f"✏️ {h['name']} ({h['time']})", f"rename_habit_{h['id']}", "primary"))
        kb_hn.row(
            cBtn("⬅️ Orqaga", "settings_info", "primary"),
            cBtn(T(uid, "btn_home"), "menu_main", "primary")
        )
