...
def callback_handler(call):
    ...
    if cdata.startswith("group_view_"):
        ...
        for mid in members:
            mu     = load_user(mid)
            m_name = mu.get("name", "?")
            m_done = any(h.get("name") == g.get("habit_name") and
                         h.get("last_done") == today
                         for h in mu.get("habits", []))
            if m_done: done_count += 1
            status = "✅" if m_done else "⬜"
            members_text += f"{status} {m_name}\n"
        bot_info = bot.get_me()
        ...
        if g.get("admin_id") == uid:
            kb_g.add(cBtn(f"🗑 Guruhni o'chirish", f"group_delete_confirm_{g_id}", "danger"))
        else:
