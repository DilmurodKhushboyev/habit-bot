...
def main_menu(uid=None):
    ...
    # Foydalanuvchi odatlari tugmalari
    if uid:
        u      = load_user(uid)
...
def send_main_menu(uid, text="Quyidagi tugmalardan foydalaning 👇"):
    u    = load_user(uid)
    sent = bot.send_message(uid, text, reply_markup=main_menu(uid))
    ...
    if cdata.startswith("toggle_"):
        ...
        for h in u.get("habits", []):
            if h["id"] == habit_id:
                ...
                if main_msg_id:
                    try:
                        bot.edit_message_reply_markup(uid, main_msg_id, reply_markup=main_menu(uid))
                    except Exception:
                        pass
                return
