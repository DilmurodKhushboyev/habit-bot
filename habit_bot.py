...
def main_menu_dict(uid=None, page=1):
    ...
    rows = []
    rows.append([
        {"text": T(uid, "btn_add"),    "callback_data": "menu_add"},
        {"text": T(uid, "btn_delete"), "callback_data": "menu_delete"},
    ])
    rows.append([
        {"text": T(uid, "btn_stats"),  "callback_data": "menu_stats"},
        {"text": T(uid, "btn_rating"), "callback_data": "menu_rating"},
    ])
    if uid:
        u      = load_user(uid)
        today  = today_uz5()
        ...
        if total_pages > 1:
            nav = []
            if page > 1:
                nav.append({"text": f"⬅️ {page-1}", "callback_data": f"main_page_{page-1}"})
            nav.append({"text": f"📄 {page}/{total_pages}", "callback_data": "noop"})
            if page < total_pages:
                nav.append({"text": f"{page+1} ➡️", "callback_data": f"main_page_{page+1}"})
            rows.append(nav)
    rows.append([
        {"text": "🛒 Bozor",             "callback_data": "menu_bozor",     "style": "primary"},
        {"text": T(uid, "btn_settings"), "callback_data": "menu_settings"},
    ])
