...
def cmd_test_onboard(msg):
    ...
    name = msg.from_user.first_name or "Admin"
    send_onboarding(uid, name)
...
def cmd_start(msg):
    ...
    if start_param.startswith("grp_"):
        ...
        if u_check.get("phone"):
            g = load_group(g_id)
            if g and uid not in g.get("members", []):
                ...
                if admin_id and admin_id != uid:
                    ...
                    save_group(g_id, g)
                    groups = u_check.get("groups", [])
                    if not any(x.get("id") == g_id for x in groups):
                        groups.append({"id": g_id, "name": g["name"], "admin_id": admin_id})
                        u_check["groups"] = groups
                        save_user(uid, u_check)
        else:
...
def callback_handler(call):
    ...
    if cdata == "admin_test_onboard":
        ...
        send_onboarding(uid, call.from_user.first_name or "Admin")
        return
    ...
    if cdata == "group_delete":
        ...
        my_groups = [g for g in u.get("groups", []) if g.get("admin_id") == uid]
        if not my_groups:
    ...
    if cdata.startswith("group_habit_add_"):
        ...
        bot.answer_callback_query(call.id)
        g = load_group(g_id)
        if not g or g.get("admin_id") != uid:
            return
        try: bot.delete_message(uid, call.message.message_id)
        except: pass
    ...
    if cdata.startswith("group_approve_"):
        ...
        if not g:
            return
        # Guruhga qo'shish
        if joiner_id not in g.get("members", []):
            g["members"].append(joiner_id)
            save_group(g_id, g)
        ...
        try:
            ...
            bot.send_message(joiner_id,
                ...
                f"_2-menyu orqali guruhingizni ko'ring!_",
                parse_mode="Markdown",
                reply_markup=kb_confirm
            )
        except: pass
    ...
    try:
        for g_doc in groups_col.find({}):
            ...
            if done_date == yesterday:
                all_done = all(done_today.get(str(mid), False) for mid in members)
                if all_done and members:
                    g["streak"] = g.get("streak", 0) + 1
                elif members:
                    # Bajarmagan a'zolarga eslatma
                    for mid in members:
                        if not done_today.get(str(mid), False):
                            try:
                                mu = load_user(mid)
                                bot.send_message(
                                    int(mid),
...
def daily_reset():
    ...
    try:
        ...
        for gdoc in groups_col.find({}):
            ...
            done_today = g.get("done_today", {})
            members    = g.get("members", [])
            not_done   = [mid for mid in members if not done_today.get(str(mid), False)]
...
def _save_new_habit(uid, u):
    ...
    def del_and_back(chat_id, mid):
        ...
        s = send_message_colored(chat_id, "➕ *Yana odat qo'shing:*", menu_dict)
        if s is None:
            kb = InlineKeyboardMarkup()
            kb.row(
                InlineKeyboardButton("📌 Oddiy", callback_data="habit_type_simple"),
            ...
            s = bot.send_message(chat_id, "➕ *Yana odat qo'shing:*", parse_mode="Markdown", reply_markup=kb)
...
def handle_text(msg):
    ...
    if state == "waiting_phone_reg":
        ...
        if len(phone_clean) >= 9:
            ...
            if not check_subscription(uid):
                ...
                return
            sent_ok = bot.send_message(uid, "✅", reply_markup=ReplyKeyboardRemove())
            def del_ok_and_lang(chat_id, mid):
                time.sleep(2)
                try: bot.delete_message(chat_id, mid)
                except: pass
                sent_lang = bot.send_message(chat_id, "🌐 Tilni tanlang / Choose language / Выберите язык:", reply_markup=lang_keyboard())
                u2 = load_user(chat_id)
                u2["lang_msg_id"] = sent_lang.message_id
                save_user(chat_id, u2)
            threading.Thread(target=del_ok_and_lang, args=(uid, sent_ok.message_id), daemon=True).start()
        else:
            bot.send_message(uid, "⚠️ Noto'g'ri raqam. Masalan: *+998901234567*", parse_mode="Markdown")
        return
    ...
    if state == "waiting_repeat_count":
        ...
        try:
            ...
            bot.send_message(uid, "⚠️ Noto'g'ri son. 1 dan 20 gacha raqam kiriting:", parse_mode="Markdown")
            return
        old_msg_id = u.pop("temp_msg_id", None)
        if old_msg_id:
            try: bot.delete_message(uid, old_msg_id)
    ...
    if state == "waiting_habit_time":
        ...
        if not re.match(r'^\d{1,2}:\d{2}$', time_text):
            bot.send_message(uid, "⚠️ Noto'g'ri format. Masalan: *07:00*", parse_mode="Markdown")
            return
        try:
            ...
            bot.send_message(uid, "⚠️ Noto'g'ri vaqt. Masalan: *07:00*", parse_mode="Markdown")
            return
        old_msg_id = u.pop("temp_msg_id", None)
        try: bot.delete_message(uid, msg.message_id)
        except: pass
    ...
    if state == "editing_habit_time":
        ...
        save_user(uid, u)
        sent_ok = bot.send_message(uid, "✅ Vaqt yangilandi!")
    ...
    if state == "editing_all_habits_time":
        ...
        try:
            ...
            bot.send_message(uid, "⚠️ Noto'g'ri vaqt.", parse_mode="Markdown")
            return
        try: bot.delete_message(uid, msg.message_id)
        except: pass
        old_msg_id = u.pop("temp_msg_id", None)
        ...
        u["state"] = None
        save_user(uid, u)
        sent_ok = bot.send_message(uid, f"✅ Barcha odatlar vaqti *{time_text}* ga o'zgartirildi!", parse_mode="Markdown")
    ...
    if state == "updating_name":
        ...
        u["state"] = None
        save_user(uid, u)
        sent_ok = bot.send_message(uid, f"✅ Ism *{new_name}* ga o'zgartirildi!", parse_mode="Markdown")
    ...
    if state == "renaming_habit":
        ...
        u["state"] = None
        save_user(uid, u)
        sent_ok = bot.send_message(uid, f"✅ Odat nomi *{new_name}* ga o'zgartirildi!", parse_mode="Markdown")
    ...
    if state == "waiting_dev_message":
        ...
        try:
            ...
            pass
        sent_ok = bot.send_message(uid, "✅ Xabaringiz dasturchiga yuborildi!")
    ...
    if state == "bozor_waiting_transfer_id":
        ...
        try:
            ...
            if target_id == uid:
                bot.send_message(uid, "⚠️ O'zingizga ball yubora olmaysiz!")
                return
            if not user_exists(target_id):
                bot.send_message(uid, "⚠️ Bu ID foydalanuvchi topilmadi.")
                return
        except ValueError:
            bot.send_message(uid, "⚠️ Faqat raqam kiriting. Masalan: *123456789*", parse_mode="Markdown")
            return
    ...
    if state == "bozor_waiting_transfer_amount":
        ...
        try:
            ...
            bot.send_message(uid, "⚠️ Musbat son kiriting. Masalan: *50*", parse_mode="Markdown")
            return
        my_points  = u.get("points", 0)
        target_id  = u.get("transfer_target_id")
        if amount > my_points:
            bot.send_message(uid, f"⚠️ Sizda faqat *{my_points} ball* bor!", parse_mode="Markdown")
            return
        old_msg_id = u.pop("temp_msg_id", None)
        if old_msg_id:
            try: bot.delete_message(uid, old_msg_id)
        ...
        try:
            ...
            pass
        sent_ok = bot.send_message(uid, f"✅ *{amount} ball* muvaffaqiyatli yuborildi!\n\n⭐ Qolgan ballingiz: {u['points']}", parse_mode="Markdown")
    ...
    if state == "bozor_waiting_subtract":
        ...
        try:
            ...
            bot.send_message(uid, "⚠️ Musbat son kiriting. Masalan: *20*", parse_mode="Markdown")
            return
        my_points = u.get("points", 0)
        ...
        u["state"]  = None
        save_user(uid, u)
        sent_ok = bot.send_message(uid, f"✅ *{deducted} ball* ayirib tashlandi.\n\n⭐ Qolgan ballingiz: {u['points']}", parse_mode="Markdown")
    ...
    if state == "admin_waiting_points_id" and uid == ADMIN_ID:
        ...
        try:
            ...
            bot.send_message(uid, "⚠️ Faqat raqam kiriting!")
            return
    ...
    if state == "admin_waiting_points_amount" and uid == ADMIN_ID:
        ...
        try:
            ...
            bot.send_message(uid, "⚠️ Son kiriting. Masalan: 100 yoki -50")
            return
        ...
        try:
            bot.send_message(
                target_id,
                f"🎁 Admin sizga *{sign}{amount} ball* berdi!\n\n⭐ Hisobingiz: {target_u['points']} ball",
                parse_mode="Markdown"
            )
        except Exception:
            pass
    ...
    if state and state.startswith("admin_waiting_reply_") and uid == ADMIN_ID:
        ...
        save_user(uid, u)
        try:
            bot.send_message(target_id, f"↩️ *Admin javobi:*\n\n{text}", parse_mode="Markdown")
    ...
    if state == "group_waiting_time":
        ...
        if not _re_g.match(r'^\d{1,2}:\d{2}$', time_text):
            bot.send_message(uid, "⚠️ Noto'g'ri format. Masalan: *21:00*", parse_mode="Markdown")
            return
        try:
            hh, mm = time_text.split(":")
            if not (0 <= int(hh) <= 23 and 0 <= int(mm) <= 59):
            ...
            bot.send_message(uid, "⚠️ Noto'g'ri vaqt. Masalan: *21:00*", parse_mode="Markdown")
            return
        try: bot.delete_message(uid, msg.message_id)
        except: pass
        old_mid = u.pop("temp_msg_id", None)
    ...
    if state == "group_habit_waiting_time":
        ...
        if not _re_gh.match(r'^\d{1,2}:\d{2}$', time_text):
            bot.send_message(uid, "⚠️ Noto'g'ri format. Masalan: *08:00*", parse_mode="Markdown")
            return
        try:
            hh, mm = time_text.split(":")
            if not (0 <= int(hh) <= 23 and 0 <= int(mm) <= 59):
            ...
            bot.send_message(uid, "⚠️ Noto'g'ri vaqt. Masalan: *08:00*", parse_mode="Markdown")
            return
        try: bot.delete_message(uid, msg.message_id)
        except: pass
        old_mid = u.pop("temp_msg_id", None)
    ...
    if state == "onboard_waiting_name":
        habit_name = text.strip()
        if len(habit_name) < 2:
            bot.send_message(uid, "⚠️ Kamida 2 harf kiriting!")
            return
        u["state"] = None
        save_user(uid, u)
        send_onboarding_time(uid, habit_name)
    ...
    if state == "onboard_waiting_time":
        ...
        if _re_ob.match(r"^\d{1,2}:\d{2}$", t):
            ...
            if 0 <= h <= 23 and 0 <= m <= 59:
                ...
                return
        bot.send_message(uid, "⚠️ Format noto\'g\'ri! Masalan: *07:30*", parse_mode="Markdown")
        return
...
:root {
  ...
  /* Neumorphism shadows */
  --sh-out:    6px 6px 14px #B8BBCA, -6px -6px 14px #FFFFFF;
  --sh-in:     inset 4px 4px 10px #B8BBCA, inset -4px -4px 10px #FFFFFF;
  --sh-sm:     3px 3px 8px #B8BBCA, -3px -3px 8px #FFFFFF;
  --sh-press:  inset 2px 2px 6px #B8BBCA, inset -2px -2px 6px #FFFFFF;
  --sh-card:   8px 8px 20px #B0B3C2, -8px -8px 20px #FFFFFF;
}
...
.header-title { font-size: 20px; font-weight: 700; color: var(--text); letter-spacing: -0.5px; }
.header-sub { font-size: 11px; color: var(--sub); font-weight: 400; }
...
.loading {
  ...
  padding: 80px 20px; gap: 16px; color: var(--sub);
  font-size: 13px;
}
.spinner {
...
.leg-empty { border: 1.5px solid var(--sub); }

.grid-header {
  display: flex; align-items: center;
  padding: 0 14px 8px; gap: 8px;
}
.gh-name { flex: 1; font-size: 10px; color: var(--sub); font-weight: 600; text-transform: uppercase; letter-spacing: 1px; }
.gh-days { display: flex; gap: 3px; }
.gh-day  { width: 18px; text-align: center; font-size: 9px; color: var(--sub); font-weight: 500; font-family: 'DM Mono', monospace; }
...
.user-row.rank-1 { box-shadow: 6px 6px 14px #B8BBCA, -6px -6px 14px #FFFFFF, inset 0 0 0 1.5px #D4963A44; }
.user-row.rank-2 { box-shadow: 6px 6px 14px #B8BBCA, -6px -6px 14px #FFFFFF, inset 0 0 0 1.5px #9AA0B044; }
.user-row.rank-3 { box-shadow: 6px 6px 14px #B8BBCA, -6px -6px 14px #FFFFFF, inset 0 0 0 1.5px #C08A4044; }

.rank-badge { font-size: 18px; width: 24px; text-align: center; }
.user-info  { flex: 1; min-width: 0; }
.user-name  { font-size: 13px; font-weight: 600; color: var(--text); }
.user-pts   { font-size: 11px; color: var(--sub); margin-top: 1px; }
.day-dots   { display: flex; gap: 3px; }
.dot {
  width: 18px; height: 18px; border-radius: 6px;
  box-shadow: var(--sh-in);
}
.dot.done  { background: var(--green); box-shadow: 2px 2px 5px #3DA86840, -2px -2px 5px #FFFFFF; }
.dot.miss  { background: var(--red);   box-shadow: 2px 2px 5px #E0505040, -2px -2px 5px #FFFFFF; }
.dot.empty { background: var(--bg); }
...
.profile-rank { font-size: 12px; color: var(--sub); margin-top: 3px; }
...
.stat-lbl { font-size: 10px; color: var(--sub); text-transform: uppercase; letter-spacing: 1px; margin-top: 2px; }

.jon-bar-top { display: flex; justify-content: space-between; font-size: 12px; color: var(--sub); margin-bottom: 8px; }
.jon-bar-bg  {
  height: 8px; border-radius: 10px;
  box-shadow: var(--sh-in);
  overflow: hidden;
}
.jon-bar-fill { height: 100%; border-radius: 10px; transition: width .6s ease; }
...
.habit-name   { flex: 1; font-size: 13px; font-weight: 600; color: var(--text); }
.habit-streak { font-size: 12px; color: var(--sub); }
...
.add-btn:active { box-shadow: var(--sh-press); transform: scale(0.98); }
...
.habit-card-icon { font-size: 22px; flex-shrink: 0; }
.habit-card-info { flex: 1; min-width: 0; }
.habit-card-name { font-size: 14px; font-weight: 600; color: var(--text); }
.habit-card-meta { font-size: 11px; color: var(--sub); margin-top: 2px; }
.habit-card-actions { display: flex; gap: 8px; }
.hbtn {
...
.hbtn:active { box-shadow: var(--sh-press); transform: scale(0.92); }

/* Modal */
.modal-overlay {
  display: none; position: fixed; inset: 0;
  background: rgba(210,213,220,0.7);
...
.modal-handle {
  ...
  background: var(--sub); opacity: 0.3;
  margin: 0 auto 20px;
}
.modal-title { font-size: 17px; font-weight: 700; text-align: center; margin-bottom: 20px; color: var(--text); }
.modal-close {
  position: absolute; right: 20px; top: 20px;
  background: var(--bg); box-shadow: var(--sh-sm);
  border: none; color: var(--sub);
...
.icon-grid {
  display: grid; grid-template-columns: repeat(8, 1fr); gap: 6px; margin-top: 6px;
}
.icon-opt {
...
.icon-opt.selected { box-shadow: var(--sh-press); background: var(--bg2); }

.save-btn {
  width: 100%; padding: 16px; border-radius: 16px;
  background: var(--bg);
  box-shadow: var(--sh-sm);
  border: none; color: var(--text); font-size: 15px; font-weight: 700;
  font-family: inherit; cursor: pointer; margin-top: 8px;
  transition: box-shadow .15s, transform .15s;
}
.save-btn:active { box-shadow: var(--sh-press); transform: scale(0.98); }
...
.today-date { font-size: 11px; color: var(--sub); letter-spacing: 1.5px; text-transform: uppercase; }
.today-prog {
  font-size: 52px; font-weight: 700; color: var(--text);
  letter-spacing: -2px; margin: 8px 0 2px;
  font-family: 'DM Mono', monospace;
}
.today-sub { font-size: 13px; color: var(--sub); }
.prog-ring-wrap { display: flex; justify-content: center; margin: 16px 0 4px; }
.prog-ring-wrap svg { filter: drop-shadow(3px 3px 8px #B8BBCA) drop-shadow(-2px -2px 6px #FFFFFF); }
...
.all-done-banner.show { display: block; animation: fadeIn .4s ease; }
@keyframes fadeIn { from { opacity: 0; transform: translateY(-8px); } to { opacity: 1; transform: translateY(0); } }
.all-done-banner .bd-icon  { font-size: 32px; }
.all-done-banner .bd-title { font-size: 15px; font-weight: 700; color: var(--text); margin-top: 8px; }
.all-done-banner .bd-sub   { font-size: 12px; color: var(--sub); margin-top: 4px; }
...
.checkin-card.done .checkin-name { color: var(--green); }
.checkin-meta { font-size: 11px; color: var(--sub); margin-top: 2px; }
...
.checkin-card.done .checkin-btn {
  box-shadow: var(--sh-press);
  background: var(--green); color: #fff; font-size: 18px;
}
.checkin-btn:active { transform: scale(0.88); }
...
.confetti-pop.show { animation: popAnim .55s ease forwards; }
@keyframes popAnim {
...
.sum-val { font-size: 28px; font-weight: 700; color: var(--text); font-family: 'DM Mono', monospace; }
.sum-lbl { font-size: 10px; color: var(--sub); text-transform: uppercase; letter-spacing: 1px; margin-top: 4px; }
...
.bar-chart-title { font-size: 12px; font-weight: 700; color: var(--sub); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 16px; }
...
.bar-fill {
  ...
  box-shadow: 2px 2px 6px #B8BBCA, -2px -2px 6px #FFFFFF;
  min-height: 4px; transition: height .5s ease;
}
.bar-lbl { font-size: 10px; color: var(--sub); font-weight: 500; }
.bar-num { font-size: 9px; color: var(--sub); font-family: 'DM Mono', monospace; }
...
.heatmap-title { font-size: 12px; font-weight: 700; color: var(--sub); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 14px; }
.heatmap-grid { display: grid; grid-template-columns: repeat(10, 1fr); gap: 5px; }
.hm-cell {
  aspect-ratio: 1; border-radius: 6px;
  background: var(--bg); box-shadow: var(--sh-in);
  transition: all .2s;
}
.hm-cell.done { background: var(--green); box-shadow: 2px 2px 5px #3DA86840, -2px -2px 5px #FFFFFF; }
.hm-cell.today-cell { box-shadow: var(--sh-in), 0 0 0 2px var(--accent2); }
.heatmap-legend { display: flex; align-items: center; gap: 6px; margin-top: 10px; font-size: 10px; color: var(--sub); }
...
.hstat-top { display: flex; align-items: center; gap: 12px; margin-bottom: 12px; }
.hstat-icon { font-size: 26px; }
.hstat-info { flex: 1; }
.hstat-name { font-size: 14px; font-weight: 700; color: var(--text); }
.hstat-sub  { font-size: 11px; color: var(--sub); margin-top: 2px; }
.hstat-pct  {
  font-size: 18px; font-weight: 700; color: var(--accent2);
  font-family: 'DM Mono', monospace;
}
.hstat-nums { display: flex; gap: 8px; margin-bottom: 10px; }
.hstat-num-box {
  flex: 1; background: var(--bg); box-shadow: var(--sh-in);
  border-radius: 12px; padding: 10px; text-align: center;
}
.hstat-num-val { font-size: 16px; font-weight: 700; color: var(--text); font-family: 'DM Mono', monospace; }
.hstat-num-lbl { font-size: 9px; color: var(--sub); text-transform: uppercase; letter-spacing: 0.5px; margin-top: 2px; }
.week-dots-row { display: flex; gap: 5px; }
.wd {
  flex: 1; height: 8px; border-radius: 4px;
  background: var(--bg); box-shadow: var(--sh-in);
}
.wd.done { background: var(--green); box-shadow: none; }
...
.rem-info-icon { font-size: 30px; }
.rem-info-text { flex: 1; }
.rem-info-title { font-size: 14px; font-weight: 700; color: var(--text); }
.rem-info-sub   { font-size: 11px; color: var(--sub); margin-top: 3px; line-height: 1.5; }
...
.rem-name  { flex: 1; font-size: 14px; font-weight: 700; color: var(--text); }
...
.toggle input:checked + .toggle-track { background: var(--green); box-shadow: inset 2px 2px 5px #3DA86860; }
.toggle-thumb {
...
.rem-body { display: flex; flex-direction: column; gap: 10px; }

/* Time picker */
.time-row { display: flex; align-items: center; gap: 10px; }
.time-lbl { font-size: 11px; color: var(--sub); font-weight: 600;
  text-transform: uppercase; letter-spacing: 1px; width: 60px; }
.time-input {
...
.time-clear:active { box-shadow: var(--sh-press); }

/* Repeat selector */
.repeat-row { display: flex; align-items: center; gap: 8px; }
.repeat-lbl { font-size: 11px; color: var(--sub); font-weight: 600;
  text-transform: uppercase; letter-spacing: 1px; width: 60px; }
.repeat-opts { display: flex; gap: 6px; flex: 1; }
.rep-btn {
...
.rep-btn.active { box-shadow: var(--sh-press); color: var(--text); background: var(--bg2); }
.rep-btn:disabled { opacity: 0.4; cursor: not-allowed; }
...
.rem-save-btn:active { box-shadow: var(--sh-press); transform: scale(0.98); }
.rem-save-btn.saved  { color: var(--green); }
...
.ach-ring-wrap { position: relative; flex-shrink: 0; }
.ach-ring-wrap svg { filter: drop-shadow(2px 2px 6px #B8BBCA) drop-shadow(-2px -2px 5px #FFF); }
.ach-ring-center {
  position: absolute; inset: 0;
  display: flex; flex-direction: column; align-items: center; justify-content: center;
}
.ach-ring-num  { font-size: 18px; font-weight: 700; color: var(--text); font-family: 'DM Mono', monospace; }
.ach-ring-lbl  { font-size: 9px; color: var(--sub); text-transform: uppercase; letter-spacing: 0.5px; }
.ach-sum-info  { flex: 1; }
.ach-sum-title { font-size: 16px; font-weight: 700; color: var(--text); }
.ach-sum-sub   { font-size: 12px; color: var(--sub); margin-top: 4px; }

.ach-cat-tabs {
  display: flex; gap: 6px; overflow-x: auto;
  scrollbar-width: none; margin-bottom: 14px; padding-bottom: 2px;
}
.ach-cat-tabs::-webkit-scrollbar { display: none; }
.ach-cat-btn {
  flex-shrink: 0; padding: 7px 14px; border-radius: 20px;
  font-size: 12px; font-weight: 600; cursor: pointer;
  border: none; background: var(--bg);
  box-shadow: var(--sh-sm); color: var(--sub);
...
.ach-card.earned {
  box-shadow: var(--sh-sm), inset 0 0 0 1.5px #4CAF7D44;
}
...
.ach-title { font-size: 14px; font-weight: 700; color: var(--text); }
.ach-desc  { font-size: 11px; color: var(--sub); margin-top: 2px; }
.ach-prog-wrap { margin-top: 8px; }
.ach-prog-bg {
...
.ach-prog-txt { font-size: 10px; color: var(--sub); margin-top: 4px; font-family: 'DM Mono', monospace; }
...
.badge-popup {
  ...
  display: flex; align-items: center; gap: 12px;
  z-index: 300; min-width: 240px; max-width: 90vw;
  transition: transform .4s cubic-bezier(.32,1.2,.5,1), opacity .3s;
  opacity: 0;
}
...
.bp-sub   { font-size: 11px; color: var(--sub); margin-top: 2px; }
...
.group-meta { font-size: 11px; color: var(--sub); margin-bottom: 12px; }
...
.member-pts  { font-size: 11px; color: var(--sub); }
...
<div class="header">
  <div class="header-top">
    <div class="logo-wrap">
      ...
      <div>
        <div class="header-title">Super Habits</div>
        <div class="header-sub">@Super_habits_bot</div>
      </div>
    </div>
    <div class="pts-chip" id="header-pts">⭐ —</div>
  </div>
</div>

<!-- PAGES -->
<div class="tabs-wrap" style="display:none" id="tabs-wrap">
  <div class="tabs" id="tabs-inner"></div>
</div>
...
<nav class="bottom-nav">
  <button class="nav-item active" id="nav-today" onclick="switchTab('today', this)">
    <span class="nav-icon">☀️</span>
    <span class="nav-label">Bugun</span>
  </button>
  <button class="nav-item" id="nav-stats" onclick="switchTab('stats', this)">
    <span class="nav-icon">📊</span>
    <span class="nav-label">Statistika</span>
  </button>
  <button class="nav-item" id="nav-achievements" onclick="switchTab('achievements', this)">
    <span class="nav-icon">🏆</span>
    <span class="nav-label">Yutuqlar</span>
  </button>
  <button class="nav-item" id="nav-reminders" onclick="switchTab('reminders', this)">
    <span class="nav-icon">🔔</span>
    <span class="nav-label">Eslatma</span>
  </button>
  <button class="nav-item" id="nav-profile" onclick="switchTab('profile', this)">
    <span class="nav-icon">👤</span>
...
<div class="badge-popup" id="badge-popup">
  ...
  <div class="bp-text">
    <div class="bp-title" id="bp-title">Yangi yutuq!</div>
    <div class="bp-sub"  id="bp-sub">Tabriklaymiz!</div>
  </div>
</div>
...
// initData dan user olish (ba'zan kechikib keladi)
function getTgUser() {
  try {
...
const user      = tgUser || { id: urlUserId || 0, first_name: 'Test' };
const initData  = tg?.initData || '';
const userId    = tgUser?.id || urlUserId || 0;
const API       = window.location.origin + '/api';

console.log('[WebApp] userId:', userId, '| tgUser:', tgUser, '| urlUserId:', urlUserId);

// ── STATE ──
let loaded = { today: false, rating: false, profile: false, habits: false, groups: false, stats: false, achievements: false, reminders: false };
let data   = {};
...
async function loadTab(tab) {
  ...
  if (tab === 'rating')       await loadRating();
...
function ringHTML(percent, size = 80) {
  ...
  return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
    <circle cx="${size/2}" cy="${size/2}" r="${r}" fill="none" stroke="#C8CBD8" stroke-width="6"/>
...
async function loadToday() {
  try {
    ...
    // Header points
    try {
      const p = await apiFetch(`profile/${userId}`);
      document.getElementById('header-pts').textContent = '⭐ ' + (p.points || 0);
    } catch(e) {}
  } catch(e) {
    document.getElementById('today-content').innerHTML =
      `<div class="empty-state"><div class="icon">⚠️</div>Ma'lumot yuklanmadi.<br><small>${e}</small></div>`;
  }
}
...
function renderToday(d) {
    ...
    cardsHtml += `
      <div class="checkin-card ${h.done ? 'done' : ''}" id="ccard-${h.id}" onclick="checkin('${h.id}', this)">
        ...
        <div class="checkin-info">
          ...
          <div class="checkin-meta">${h.time !== 'vaqtsiz' ? '⏰ ' + h.time + ' · ' : ''}🔥 ${h.streak} kun</div>
        </div>
        <button class="checkin-btn" id="cbtn-${h.id}">${h.done ? '✓' : ''}</button>
  ...
  document.getElementById('today-content').innerHTML = `
    <div class="all-done-banner ${allDone ? 'show' : ''}" id="all-done-banner">
      ...
      <div class="bd-title">Barchasi bajarildi!</div>
      <div class="bd-sub">Ajoyib! Streak davom etmoqda 🔥</div>
    </div>

    <div class="today-hero">
      <div class="today-date">${dateStr}</div>
      <div class="prog-ring-wrap" id="prog-ring">${ringHTML(percent)}</div>
      <div class="today-sub" id="prog-sub">${done_count} / ${total} odat bajarildi</div>
    </div>

    <div class="section-title">Bugungi odatlar</div>
    ${cardsHtml || '<div class="empty-state"><div class="icon">📋</div>Hali odat qo\\'shilmagan</div>'}
...
async function checkin(hid, cardEl) {
  ...
  try {
    ...
    if (metaEl) {
      const tp = metaEl.innerHTML.includes('⏰') ? metaEl.innerHTML.split('·')[0] + '· ' : '';
      metaEl.innerHTML = `${tp}🔥 ${result.streak} kun`;
    }
    ...
    // Ring yangilash
    if (data.today) {
      data.today.done_count += isDone ? 1 : -1;
      ...
      if (subEl) subEl.textContent = `${data.today.done_count} / ${data.today.total} odat bajarildi`;
    ...
    showTodayToast(isDone ? '✅ ' + result.msg : '↩️ Bekor qilindi');
    loaded.profile = false; loaded.rating = false; loaded.achievements = false;
    // Yangi badge popup
    if (isDone && result.new_badges && result.new_badges.length) {
      result.new_badges.forEach(b => showBadgePopup(b.icon, b.title, 'Yangi yutuq qozonildi!'));
    }
  } catch(e) {
    showTodayToast('❌ Xato: ' + e.message, true);
  } finally {
...
async function loadReminders() {
  try {
    ...
    document.getElementById('reminders-content').innerHTML =
      `<div class="empty-state"><div class="icon">⚠️</div>Ma'lumot yuklanmadi.</div>`;
  }
}
...
function renderReminders(habits) {
  if (!habits.length) {
    document.getElementById('reminders-content').innerHTML =
      `<div class="empty-state"><div class="icon">📋</div>Hali odat qo'shilmagan.<br>Avval odat qo'shing!</div>`;
    return;
  }

  const cardsHtml = habits.map(h => {
    const enabled = h.reminder_enabled !== false;
    const time    = h.time && h.time !== 'vaqtsiz' ? h.time : '';
    const repeat  = h.repeat || 'daily';
    const reps    = [
      {id:'daily',    label:'Har kun'},
      {id:'weekdays', label:'Ish kuni'},
      {id:'weekends', label:'Dam olish'},
    ];
    const repBtns = reps.map(r =>
      `<button class="rep-btn ${repeat===r.id?'active':''} ${!enabled?'':''}
        id="rep-${h.id}-${r.id}"
        onclick="setRepeat('${h.id}','${r.id}')"
        ${!enabled ? 'disabled' : ''}>${r.label}</button>`
    ).join('');

    return `
      <div class="rem-card ${!enabled ? 'disabled' : ''}" id="remcard-${h.id}">
        <div class="rem-top">
          <div class="rem-icon">${h.icon || '✅'}</div>
        ...
        <div class="rem-body" id="rembody-${h.id}" style="${!enabled ? 'opacity:.4;pointer-events:none' : ''}">
          <div class="time-row">
            <span class="time-lbl">Vaqt</span>
            <input class="time-input" type="time" id="time-${h.id}"
              value="${time}" placeholder="--:--">
            <button class="time-clear" onclick="clearTime('${h.id}')" title="Vaqtni o'chirish">✕</button>
          </div>
          <div class="repeat-row">
            <span class="repeat-lbl">Kun</span>
            <div class="repeat-opts">${repBtns}</div>
          </div>
          <button class="rem-save-btn" id="rsave-${h.id}" onclick="saveReminder('${h.id}')">
            💾 Saqlash
          </button>
        </div>
      </div>`;
  ...
  document.getElementById('reminders-content').innerHTML = `
    <div class="rem-info-card">
      <div class="rem-info-icon">🔔</div>
      <div class="rem-info-text">
        <div class="rem-info-title">Eslatma sozlamalari</div>
        <div class="rem-info-sub">Har bir odat uchun vaqt va kun turini belgilang. Toggle orqali o'chiring.</div>
      </div>
    </div>
    <div class="section-title">Odatlar (${habits.length})</div>
    ${cardsHtml}
...
function setRepeat(hid, val) {
  ['daily','weekdays','weekends'].forEach(r => {
    const btn = document.getElementById(`rep-${hid}-${r}`);
    if (btn) btn.classList.toggle('active', r === val);
  });
}
...
async function saveReminder(hid) {
  ...
  if (saveBtn) { saveBtn.textContent = '⏳ Saqlanmoqda...'; saveBtn.disabled = true; }
  ...
  try {
    ...
    showRemToast(enabled && time ? `🔔 ${time} da eslatma o'rnatildi` : '🔕 Eslatma o'chirildi');
    loaded.habits = false;
  } catch(e) {
    if (saveBtn) { saveBtn.textContent = '💾 Saqlash'; saveBtn.disabled = false; }
    showRemToast('❌ Xato: ' + e.message, true);
  }
}
...
function renderAchievements(d, filter = 'all') {
  ...
  const sumHtml = `
    <div class="ach-summary">
      ...
      <div class="ach-sum-info">
        <div class="ach-sum-title">Yutuqlarim</div>
        <div class="ach-sum-sub">${earned_count} / ${total_count} qozonildi</div>
      </div>
    </div>`;

  // Category tabs
  const allCats = [{id:'all',label:'Barchasi'}, ...cats];
  ...
  const cardsHtml = sorted.map(a => {
    ...
    return `
      <div class="ach-card ${a.earned ? 'earned' : 'locked'}">
        <div class="ach-badge">
          ${a.icon}
          ${a.earned ? '<div class="ach-badge-check">✓</div>' : ''}
        </div>
        <div class="ach-info">
  ...
  document.getElementById('achievements-content').innerHTML = `
    ...
    ${cardsHtml || '<div class="empty-state"><div class="icon">🔒</div>Bu kategoriyada hali yutuq yo\\'q</div>'}`;
}
...
function renderStats(d) {
  const { summary, weekly, heatmap, days_30, habit_stats, today } = d;
  ...
  const sumHtml = `
    <div class="summary-grid">
      <div class="sum-card">
        ...
        <div class="sum-val">${summary.streak}</div>
        <div class="sum-lbl">Streak</div>
      </div>
      <div class="sum-card">
        <div class="sum-icon">⭐</div>
        <div class="sum-val">${summary.points}</div>
        <div class="sum-lbl">Ball</div>
      </div>
      <div class="sum-card">
        <div class="sum-icon">✅</div>
        <div class="sum-val">${summary.active_days_30}</div>
        <div class="sum-lbl">Faol kun (30)</div>
      </div>
      <div class="sum-card">
        <div class="sum-icon">📋</div>
        <div class="sum-val">${summary.total_habits}</div>
        <div class="sum-lbl">Odat</div>
      </div>
    </div>`;
  ...
  const barsHtml = weekly.map((w, i) => {
    ...
    return `
      <div class="bar-col">
        <div class="bar-num">${w.count > 0 ? w.count : ''}</div>
  ...
  const barChartHtml = `
    <div class="bar-chart">
      <div class="bar-chart-title">Haftalik bajarilish</div>
  ...
  const heatmapHtml = `
    <div class="heatmap-wrap">
      <div class="heatmap-title">30 kunlik faollik</div>
      <div class="heatmap-grid">${hmCells}</div>
      <div class="heatmap-legend">
        <div class="hm-leg" style="background:var(--bg);box-shadow:var(--sh-in)"></div> Bajarilmadi
        <div class="hm-leg" style="background:var(--green);margin-left:10px"></div> Bajarildi
      </div>
    </div>`;

  // ── Har bir odat ──
  let habitCardsHtml = '';
  (habit_stats || []).forEach(h => {
    const pctColor = h.percent >= 80 ? '#4CAF7D' : h.percent >= 50 ? '#5B8DEF' : '#E07040';
    const wdDots   = h.week_dots.map(v => `<div class="wd ${v ? 'done' : ''}"></div>`).join('');
    habitCardsHtml += `
      <div class="hstat-card">
        <div class="hstat-top">
          ...
          <div class="hstat-info">
            ...
            <div class="hstat-sub">🔥 ${h.streak} kun streak</div>
          </div>
          <div class="hstat-pct" style="color:${pctColor}">${h.percent}%</div>
        </div>
        <div class="hstat-nums">
          <div class="hstat-num-box">
            <div class="hstat-num-val">${h.done_7}</div>
            <div class="hstat-num-lbl">7 kun</div>
          </div>
          <div class="hstat-num-box">
            <div class="hstat-num-val">${h.done_30}</div>
            <div class="hstat-num-lbl">30 kun</div>
          </div>
          <div class="hstat-num-box">
            <div class="hstat-num-val">${h.total_done}</div>
            <div class="hstat-num-lbl">Jami</div>
          </div>
        </div>
        <div class="week-dots-row">${wdDots}</div>
      </div>`;
  });

  document.getElementById('stats-content').innerHTML = `
    <div class="section-title">Umumiy</div>
    ${sumHtml}
    ${barChartHtml}
    ${heatmapHtml}
    <div class="section-title">Har bir odat</div>
    ${habitCardsHtml || '<div class="empty-state"><div class="icon">📋</div>Hali odat qo\\'shilmagan</div>'}`;
}

// ── REYTING ──
async function loadRating() {
  try {
    const d = await apiFetch('rating');
    data.rating = d;
    renderRating(d);
  } catch(e) {
...
function renderRating(d) {
  const { today, days, day_labels: lbls, users } = d;
  const rankColors = ['#D4963A','#9AA0B0','#C08A40'];
  let daysHtml = lbls.map(l => `<div class="gh-day">${l}</div>`).join('');
  let rowsHtml = '';

  users.forEach((u, i) => {
    const rc = rankColors[i] || '#8A8D9A';
    const rankClass = i < 3 ? `rank-${i+1}` : '';
    const medal = ['🥇','🥈','🥉'][i] || (i+1);
    let dots = '';
    days.forEach(dstr => {
      if (dstr > today || dstr < u.bot_start) dots += `<div class="dot empty"></div>`;
      else if (u.done_log && u.done_log[dstr]) dots += `<div class="dot done"></div>`;
      else dots += `<div class="dot miss"></div>`;
    });
    rowsHtml += `
      <div class="user-row ${rankClass}">
        ...
        <div class="user-info">
          ...
          <div class="user-pts">${u.points} ball</div>
        </div>
  ...
  const now = new Date();
  const monthLbl = oy[now.getMonth()+1] + ' ' + now.getFullYear();

  document.getElementById('rating-content').innerHTML = `
    <div class="section-title">Top 10 · ${monthLbl}</div>
    <div class="legend">
      <div class="leg-item"><div class="leg-dot leg-green"></div>Bajarildi</div>
      <div class="leg-item"><div class="leg-dot leg-red"></div>Bajarilmadi</div>
      <div class="leg-item"><div class="leg-dot leg-empty"></div>Ma'lumot yo'q</div>
    </div>
    <div class="grid-header">
      <div class="gh-name">Foydalanuvchi</div>
      <div class="gh-days">${daysHtml}</div>
    </div>
    ${rowsHtml}`;
}
  ...
  (d.habits || []).forEach(h => {
    habitsHtml += `
      <div class="habit-item">
        ...
        <div class="habit-streak">🔥 ${h.streak || 0} kun</div>
      </div>`;
  });
  ...
  document.getElementById('profile-content').innerHTML = `
    <div class="profile-card">
      <div class="profile-top">
        ...
        <div>
          <div class="profile-name">${d.name || 'Foydalanuvchi'}</div>
          <div class="profile-rank">${d.rank ? '#'+d.rank+' o\\'rinda' : ''} ${d.is_vip ? '💎 VIP' : ''}</div>
        </div>
      </div>
      <div class="stats-grid">
        <div class="stat-box"><div class="stat-val">${d.points || 0}</div><div class="stat-lbl">Ball</div></div>
        <div class="stat-box"><div class="stat-val">${d.streak || 0}</div><div class="stat-lbl">Streak</div></div>
        <div class="stat-box"><div class="stat-val">${(d.habits||[]).length}</div><div class="stat-lbl">Odat</div></div>
      </div>
      <div class="jon-bar-top"><span>❤️ Jon</span><span>${jon}%</span></div>
      <div class="jon-bar-bg">
        <div class="jon-bar-fill" style="width:${jon}%;background:${jonColor}"></div>
      </div>
    </div>
    <div class="section-title">Odatlarim</div>
    <div class="habits-list">${habitsHtml || '<div class="empty-state"><div class="icon">📋</div>Hozircha odat yo\\'q</div>'}</div>`;
}
...
function renderHabits(habits) {
      ...
      <div class="habit-card" id="hcard-${h.id}">
        <div class="habit-card-top">
          ...
          <div class="habit-card-info">
            ...
            <div class="habit-card-meta">⏰ ${h.time||'vaqtsiz'} &nbsp;🔥 ${h.streak} kun</div>
          </div>
          <div class="habit-card-actions">
            <button class="hbtn" onclick="openEdit('${h.id}','${h.name.replace(/'/g,"\\\\'")}','${h.icon||'✅'}','${h.time||'vaqtsiz'}')">✏️</button>
            <button class="hbtn" onclick="deleteHabit('${h.id}')">🗑️</button>
          </div>
        </div>
      </div>`;
  });
  ...
  document.getElementById('habits-content').innerHTML = `
    <div>
      <button class="add-btn" onclick="openAdd()">＋ Yangi odat qo'shish</button>
      <div class="section-title">Odatlarim (${habits.length})</div>
      ${rows || '<div class="empty-state"><div class="icon">📋</div>Hozircha odat yo\\'q</div>'}
    </div>
    ...
    <div class="modal-overlay" id="habit-modal">
      <div class="modal">
        ...
        <button class="save-btn" onclick="saveHabit()">Saqlash</button>
      </div>
    </div>`;
}
...
function openAdd() {
  editingHabitId = null;
  document.getElementById('modal-title').textContent = 'Yangi odat';
...
function openEdit(id, name, icon, time) {
  editingHabitId = id;
  document.getElementById('modal-title').textContent = 'Odatni tahrirlash';
...
function closeModal() { document.getElementById('habit-modal').classList.remove('open'); }
...
async function saveHabit() {
  ...
  const time = document.getElementById('h-time').value.trim() || 'vaqtsiz';
  const icon = document.querySelector('.icon-opt.selected')?.dataset.icon || '✅';
  if (!name) { showToast('Nom kiriting!', true); return; }
  try {
    if (editingHabitId) {
      await fetch(`${API}/habits/${userId}/${editingHabitId}`, {
        method: 'PUT',
        headers: {'Content-Type':'application/json','X-User-Id':userId},
        body: JSON.stringify({name, icon, time})
      });
      showToast('✅ Saqlandi!');
    } else {
      await fetch(`${API}/habits/${userId}`, {
        method: 'POST',
        headers: {'Content-Type':'application/json','X-User-Id':userId},
        body: JSON.stringify({name, icon, time})
      });
    ...
    loaded.habits = false;
    await loadHabits();
  } catch(e) { showToast('Xato yuz berdi!', true); }
}

async function deleteHabit(id) {
  if (!confirm('Odatni o\\'chirasizmi?')) return;
  try {
    await fetch(`${API}/habits/${userId}/${id}`, {
...
function renderGroups(d) {
  if (!d.groups || d.groups.length === 0) {
    document.getElementById('groups-content').innerHTML =
      `<div class="empty-state"><div class="icon">👥</div>Siz hech qanday guruhga a'zo emassiz</div>`;
    return;
  }
  let html = '';
  d.groups.forEach(g => {
    let membersHtml = (g.members || []).slice(0,5).map((m,i) =>
      `<div class="member-row">
        <div class="member-name">${['🥇','🥈','🥉'][i]||'•'} ${m.name}</div>
        <div class="member-pts">${m.points} ball</div>
       </div>`
    ).join('');
    html += `
      <div class="group-card">
        <div class="group-name">👥 ${g.name}</div>
        <div class="group-meta">${g.member_count} a'zo</div>
        <div class="group-members">${membersHtml}</div>
      </div>`;
  });
  document.getElementById('groups-content').innerHTML =
    `<div class="section-title">Mening guruhlarim</div>${html}`;
}
...
async function startApp() {
  if (!userId || userId == 0 || userId === '0') {
    document.getElementById('today-content').innerHTML =
      '<div class="empty-state"><div class="icon">⚠️</div>Foydalanuvchi aniqlanmadi.<br><small>Iltimos botdan qayta oching.</small></div>';
    // 1.5 soniya kutib qayta urinamiz (Telegram initData kechikishi uchun)
    await new Promise(r => setTimeout(r, 1500));
    const tgUser2 = getTgUser();
    const uid2 = tgUser2?.id || new URLSearchParams(window.location.search).get('uid') || 0;
    if (uid2 && uid2 != 0) {
