...
def cmd_start(msg):
    ...
    # Yangi foydalanuvchi tekshiruvi
    is_new = str(uid) not in d or not d[str(uid)].get("phone")
    ...
    if state in ("admin_bc_private", "admin_bc_groups", "admin_bc_all") and uid == ADMIN_ID:
        ...
        for target_uid, udata in users.items():
            if state == "admin_bc_groups":
                ...
                if chat_type != "group":
                    continue
            # admin_bc_private va admin_bc_all da barcha shaxsiy foydalanuvchilarga yuboriladi
            try:
        ...
        u["state"] = None
        save_data(d)
        bot.send_message(uid, f"✅ Habar {sent} ta foydalanuvchiga yuborildi.")
        bot.send_message(uid, "🛠 *Admin panel*", parse_mode="Markdown", reply_markup=admin_menu())
        return
...
def callback_handler(call):
    ...
    if cdata == "admin_users":
        ...
        parts = []
        current = f"👥 Foydalanuvchilar: {len(users)} ta\n"
        current += "▬" * 20 + "\n\n"
