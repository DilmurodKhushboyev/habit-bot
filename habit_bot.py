...
def menu2_dict(uid=None):
    ...
    if uid:
        ...
        for g in groups:
            g_id   = g.get("id", "")
            g_name = g.get("name", "Guruh")
            rows.append([
                {"text": f"👥 {g_name}", "callback_data": f"group_view_{g_id}", "style": "primary"}
            ])
    rows.append([
        {"text": "🛒 Bozor",             "callback_data": "menu_bozor",     "style": "primary"},
...
def cmd_start(msg):
    ...
    if start_param.startswith("grp_"):
        ...
        if u_check.get("phone"):
            # Ro'yxatdan o'tgan — hoziroq qo'shamiz
            g = load_group(g_id)
                ...
                try:
                    ...
                    if admin_id and admin_id != uid:
                        bot.send_message(admin_id,
                            f"👋 *{u_check.get('name','Yangi azо')}* guruhga qo'shildi!\n"
                            f"👥 *{g['name']}*",
                            parse_mode="Markdown"
                        )
                except: pass
...
def callback_handler(call):
    ...
    if cdata == "group_rating":
        ...
        for mid in g.get("members", []):
            mu     = load_user(mid)
            m_done = sum(1 for h in mu.get("habits", [])
                         if h.get("name") == g.get("habit_name") and
                         h.get("last_done") == today_uz5())
            members_data.append((mu.get("name","?"), m_done, mu.get("streak",0)))
        members_data.sort(key=lambda x: x[1], reverse=True)
        text = f"🏆 *{g['name']} — Reyting*\n\n"
        for i, (name, done, streak) in enumerate(members_data, 1):
            medal = "🥇" if i==1 else "🥈" if i==2 else "🥉" if i==3 else f"{i}."
            status = "✅" if done else "⬜"
            text  += f"{medal} {status} *{name}* — 🔥{streak}\n"
        kb_back = InlineKeyboardMarkup()
        kb_back.add(cBtn("⬅️ Orqaga", "menu2_open", "primary"))
        sent = bot.send_message(uid, text, parse_mode="Markdown", reply_markup=kb_back)
        u["main_msg_id"] = sent.message_id
