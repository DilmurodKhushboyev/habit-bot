...
def handle_text(msg):
    ...
    if (state not in ("admin_bc_private", "admin_bc_groups", "admin_bc_all") and
        ...
        not (state and state.startswith("admin_waiting_points"))) and uid != ADMIN_ID:
        if not check_subscription(uid):
            send_sub_required(uid)
            return
    ...
    if state == "admin_waiting_points_amount" and uid == ADMIN_ID:
        ...
        try:
            sent_notif = bot.send_message(target_uid_int, user_notif, parse_mode="Markdown")
            # Foydalanuvchi ko'rsa 5 soniyada o'chsin
            def del_notif(t_id, n_mid):
                time.sleep(5)
                try: bot.delete_message(t_id, n_mid)
                except: pass
            threading.Thread(target=del_notif, args=(target_uid_int, sent_notif.message_id), daemon=True).start()
        except Exception:
            pass
        return
