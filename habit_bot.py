...
def send_onboarding(uid, name):
    ...
    text1 = (
        ...
        f"*Ball* — har bajarganingizda +5 ball.\n"
        f"*Guruh* — do'stlar bilan birga, uyat kuchli motivator! 😄\n\n"
        f"━━━━━━━━━━━━━━━━\n"
...
ONBOARD_EXAMPLES = {
    ...
    "mind": [
        ...
        ("🙏 Namoz",                   "Namoz"),
        ("😴 23:00 da uxlash",         "23:00 da uxlash"),
    ],
    "work": [
...
def callback_handler(call):
    ...
    if cdata in ("admin_bc_private", "admin_bc_groups", "admin_bc_all"):
        ...
        u["state"] = cdata
        save_user(uid, u)
        # Tekshiruv: state to'g'ri saqlandimi?
        u2 = load_user(uid)
        print(f"[DEBUG bc_set] state saqlandi: {u2.get('state')!r}")
        bot.answer_callback_query(call.id)
        try: bot.delete_message(uid, call.message.message_id)
        except Exception: pass
...
def _run_broadcast(admin_uid, bc_chat_id, msg_ids, state):
    ...
    if uid == ADMIN_ID:
        print(f"[DEBUG broadcast] uid={uid}, state={state!r}, content_type={msg.content_type}")
    if state in ("admin_bc_private", "admin_bc_groups", "admin_bc_all") and uid == ADMIN_ID:
        ...
        # Media group (album) bo'lsa — thread ichida yig'ish (bot bloklanmasin)
        if media_grp_id:
            ...
            if buf["processing"]:
                return
            buf["processing"] = True
            # Thread ichida 1.5 sek kutib broadcast qilamiz
            bc_state   = state
            bc_uid     = uid
            bc_chat_id_cap = bc_chat_id
            def _do_album_broadcast():
                time.sleep(1.5)
                final_ids = bot._bc_album_buffer.pop(media_grp_id, {}).get("ids", [msg.message_id])
                _run_broadcast(bc_uid, bc_chat_id_cap, final_ids, bc_state)
            ...
            threading.Thread(
                ...
                args=(bc_uid_cap, bc_chat_cap, bc_ids_cap, bc_state_cap),
                daemon=True
            ).start()
        return
