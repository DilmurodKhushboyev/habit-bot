...
def show_rating(uid):
    ...
    if top10:
        for i, (name, points, username, target_uid) in enumerate(top10):
            ...
            text += f"{medals[i]} {link} — {points} ball,  {je} {jon_val}%\n"
    else:
    ...
    for uid, udata in users.items():
        ...
        for habit in udata.get("habits", []):
            ...
            if hab_type == "repeat":
                ...
                # Oddiy: kecha bajarilmagan bo'lsa missed++
                if habit.get("last_done") != yesterday and habit.get("last_done") is not None:
                    # Avval bajarilgan bo'lgan odat kecha o'tkazib yuborilgan
                    habit["total_missed"] = habit.get("total_missed", 0) + 1
                    changed = True
                elif habit.get("last_done") is None and habit.get("total_done", 0) > 0:
                    # Hech bajarilmagan emas, ammo bugun ham bajarilmagan
                    habit["total_missed"] = habit.get("total_missed", 0) + 1
                    changed = True
        # Jon hisoblash
