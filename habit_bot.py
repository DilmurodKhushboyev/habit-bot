...
def cmd_start(msg):
    uid  = msg.from_user.id
    # Referal linkni tekshirish: /start ref_12345678
    start_param = msg.text.split()[1] if len(msg.text.split()) > 1 else ""
    if start_param.startswith("join_"):
        ch_id = start_param[5:]
        ch    = get_challenge(ch_id)
        if not ch or ch.get("finished"):
            bot.send_message(uid, "❌ Bu challenge topilmadi yoki tugagan.")
            return
        if uid in ch.get("members", []):
            # Allaqachon a'zo — challengeni ko'rsatish
            text = challenge_status_text(ch, viewer_uid=uid)
            from datetime import timezone, timedelta
            tz_uz   = timezone(timedelta(hours=5))
            today_s = datetime.now(tz_uz).strftime("%Y-%m-%d")
            done_today = today_s in ch.get("progress", {}).get(str(uid), [])
            kb_cv = InlineKeyboardMarkup()
            if ch.get("active") and not done_today:
                kb_cv.add(cBtn("✅ Bugun bajardim!", f"challenge_done_{ch_id}", "primary"))
            kb_cv.add(cBtn(T(uid, "btn_home"), "menu_main", "primary"))
            bot.send_message(uid, text, parse_mode="Markdown", reply_markup=kb_cv)
            return
        if len(ch.get("members", [])) >= 5:
            bot.send_message(uid, "❌ Bu challengeda joy qolmadi (maksimal 5 kishi).")
            return
        # Qo'shish
        ...
        tz_uz   = timezone(timedelta(hours=5))
        today_s = datetime.now(tz_uz).strftime("%Y-%m-%d")
        if len(ch["members"]) >= 2 and not ch.get("active"):
            dur   = ch["duration"]
            end_d = (date.today() + timedelta(days=dur-1)).isoformat()
            ch["active"]     = True
            ch["start_date"] = today_s
            ch["end_date"]   = end_d
            save_challenge(ch)
            for m in ch["members"]:
                try:
                    bot.send_message(m,
                        f"🚀 *{ch['name']}* boshlandi!\n"
                        f"👥 A'zolar: {len(ch['members'])} ta\n"
                        f"⏳ {dur} kun",
                        parse_mode="Markdown")
                except: pass
        else:
            save_challenge(ch)
            u_load = load_user(uid)
            u_name = u_load.get("name", "?")
            try:
                bot.send_message(ch["creator"],
                    f"👥 *{u_name}* challengega qo'shildi!\n"
                    f"A'zolar: {len(ch['members'])}/5",
                    parse_mode="Markdown")
            except: pass
            # Yangi a'zoga xabar
            text = challenge_status_text(ch, viewer_uid=uid)
    ...
    if start_param.startswith("ref_"):
        try:
            ...
            if referrer_id != uid:  # O'zini taklif qilolmaydi
                u_check = load_user(uid)
                # Faqat yangi foydalanuvchi bonus oladi
...
def handle_contact(msg):
    ...
    if referrer_id and not u.get("ref_used"):
        ...
        try:
            ...
            ref_total = len(refs)
            # Chegara bonuslarini tekshirish
            milestone_msg = ""
            milestones = {5: "🛡 Streak himoyasi (1 ta) qo'shildi!", 10: "⭐ +100 ball bonus!", 20: "💎 VIP nishon qo'shildi!"}
            if ref_total in milestones:
                milestone_msg = f"\n\n🏆 *Chegara bonusi:* {milestones[ref_total]}"
                if ref_total == 5:
            ...
            bot.send_message(referrer_id,
                ...
                f"*👥 Jami taklif qilganlar:* {ref_total} ta"
                + milestone_msg,
                parse_mode="Markdown")
        except Exception:
            pass
...
@bot.message_handler(func=lambda m: True, content_types=[
    ...
    "animation","video_note","location","venue","poll","dice"
])
def handle_text(msg):
    uid  = msg.from_user.id
    ...
    u     = load_user(uid)
    state = u.get("state")
    # Broadcast bo'lmasa va text yo'q bo'lsa — chiqish
    if not text and state not in ("admin_bc_private", "admin_bc_groups", "admin_bc_all"):
        return
    ...
    if state in ("admin_bc_private", "admin_bc_groups", "admin_bc_all") and uid == ADMIN_ID:
        ...
        wait_sent  = bot.send_message(uid, "📤 Habar yuborilmoqda, iltimos kuting...")
        def do_broadcast(bc_st, adm_id, m_id, wait_id):
            ...
            for target_uid, udata in users.items():
                if int(target_uid) == ADMIN_ID:
                    continue
                # Chat turi bo'yicha filter
                chat_type = udata.get("chat_type", "private")
                if bc_st == "admin_bc_private" and chat_type != "private":
                    continue
                if bc_st == "admin_bc_groups" and chat_type != "group":
                    continue
                try:
                    bot.copy_message(int(target_uid), adm_id, m_id)
                    sent_count += 1
                    ...
                    if "429" in err_str:
                        ...
                        try:
                            ...
                            failed += 1
                            failed_list.append(f"{udata.get('name','?')} ({target_uid}): rate limit")
                    else:
                        failed += 1
                        failed_list.append(f"{udata.get('name','?')} ({target_uid}): botni bloklagan")
                        print(f"[broadcast] xato: {target_uid} — {e}")
            try: bot.delete_message(adm_id, wait_id)
            except: pass
            u2 = load_user(adm_id)
            u2["bc_failed_list"] = failed_list
            save_user(adm_id, u2)
            result = (
                f"*📊 Broadcast natija:*\n\n"
            ...
            bot.send_message(adm_id, result, parse_mode="Markdown", reply_markup=kb_bc)
        threading.Thread(target=do_broadcast, args=(bc_state, admin_id, msg_id, wait_sent.message_id), daemon=True).start()
        return
    ...
    if state == "waiting_habit_repeat_times":
        ...
        def del_added_rep(chat_id, mid, first):
            ...
            sent2 = send_message_colored(chat_id, main_text, main_menu_dict(chat_id))
            if sent2 is None:
    ...
    if state == "bozor_waiting_transfer_id":
        ...
        cancel_kb = InlineKeyboardMarkup()
        cancel_kb.row(
            cBtn("⬅️ Orqaga", "bozor_transfer", "primary"),
    ...
    if state == "bozor_waiting_transfer_amount":
        ...
        if amount > my_balls:
            ...
            def del_noball(chat_id, mid):
                ...
                kb_b.add(cBtn(T(chat_id, "btn_home"),         "menu_main", "primary"))
                s2 = bot.send_message(chat_id, f"🛒 *Bozor —* bu sizning ballaringiz bilan qilinadigan barcha ammalar joyi.\n\n*⭐ Sizning balingiz:* {balls2} ball\n\n*💸 Ballarni ayirboshlash —* yaqin insoniga ball yuborish\n*🔴 Ballarimni 0 ga tushirish —* barcha ballarni nollash\n*➖ Ballarimdan olib tashlash —* ma'lum miqdorni ayirish", parse_mode="Markdown", reply_markup=kb_b)
        ...
        sent2 = bot.send_message(uid, f"🛒 *Bozor —* bu sizning ballaringiz bilan qilinadigan barcha ammalar joyi.\n\n*⭐ Sizning balingiz:* {balls2} ball\n\n*💸 Ballarni ayirboshlash —* yaqin insoniga ball yuborish\n*🔴 Ballarimni 0 ga tushirish —* barcha ballarni nollash\n*➖ Ballarimdan olib tashlash —* ma'lum miqdorni ayirish", parse_mode="Markdown", reply_markup=kb_b)
        u2 = load_user(uid)
        u2["main_msg_id"] = sent2.message_id
        save_user(uid, u2)
        return
    ...
    if state == "bozor_waiting_subtract":
        ...
        kb_b2.add(InlineKeyboardButton("🔴 Ballarimni 0 ga tushirish", callback_data="bozor_reset_confirm"))
        kb_b2.add(InlineKeyboardButton("➖ Ballarimdan olib tashlash",  callback_data="bozor_subtract"))
        kb_b2.add(cBtn(T(uid, "btn_home"),             "menu_main", "primary"))
    ...
    if state == "challenge_waiting_name":
        ch_name = text.strip()
        info_mid = u.pop("temp_msg_id", None)
        try: bot.delete_message(uid, msg.message_id)
        except: pass
        if info_mid:
            try: bot.delete_message(uid, info_mid)
            except: pass
        u["temp_challenge_name"] = ch_name
        u["state"] = "challenge_waiting_duration"
        save_user(uid, u)
        # Davomiylik tanlash
        kb_dur = InlineKeyboardMarkup()
        kb_dur.row(
            InlineKeyboardButton("7 kun",  callback_data="challenge_duration_7"),
            InlineKeyboardButton("14 kun", callback_data="challenge_duration_14"),
        )
        kb_dur.row(
            InlineKeyboardButton("21 kun", callback_data="challenge_duration_21"),
            InlineKeyboardButton("30 kun", callback_data="challenge_duration_30"),
        )
        kb_dur.add(InlineKeyboardButton(T(uid, "btn_cancel"), callback_data="cancel_to_main"))
        sent = bot.send_message(
            uid,
            f"👥 *{ch_name}*\n\n⏳ Necha kun davom etsin?",
    ...
    if state == "renaming_habit":
        ...
        def _del_ok(chat_id, mid):
            import time as _t; _t.sleep(5)
            try: bot.delete_message(chat_id, mid)
            except: pass
        threading.Thread(target=_del_ok, args=(uid, sent_ok.message_id), daemon=True).start()
        # change_habit_name (odat nomi tahrirlash ro'yxati) ga qaytish
        def _sk(h):
            t = h.get("time", "23:59")
            try:
                hh, mm = t.split(":")
        ...
        sorted_h = sorted(u.get("habits", []), key=_sk)
...
def build_weekly_report_text(uid, report):
    """Saqlangan hisobot ma'lumotidan matn yaratish"""
...
def send_challenge_reminders():
    """Har kuni 20:00 da bajarilmagan challenge a'zolariga eslatma"""
    from datetime import timezone, timedelta
    tz_uz   = timezone(timedelta(hours=5))
    today_s = datetime.now(tz_uz).strftime("%Y-%m-%d")
    active_chs = list(challenges_col.find({"active": True, "finished": False}))
    for ch in active_chs:
        members  = ch.get("members", [])
        progress = ch.get("progress", {})
        # Kim bajargan?
        done_uids   = [m for m in members if today_s in progress.get(str(m), [])]
        undone_uids = [m for m in members if today_s not in progress.get(str(m), [])]
        if not undone_uids:
            continue
        # Bajarganlar ro'yxati
        done_names = []
        for d_uid in done_uids:
            u_d = load_user(d_uid)
            done_names.append(u_d.get("name", "?"))
        done_txt = ", ".join(done_names) if done_names else "hali hech kim"
        # Bajarmaganlarga eslatma
        for m_uid in undone_uids:
            try:
                bot.send_message(
                    m_uid,
                    f"⏰ *{ch['name']}* — bugun hali bajarmaganingiz!\n\n"
                    f"✅ Bajarganlar: {done_txt}\n"
                    f"⏳ Tugash: 23:59",
                    parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup().add(
                        InlineKeyboardButton("👥 Challengega o'tish",
                            callback_data=f"challenge_view_{ch['_id']}")
                    )
                )
                time.sleep(0.05)
            except Exception:
...
def send_weekly_reports():
    """Har dushanba 09:00 (UTC+5) da haftalik hisobot yuborish"""
    from datetime import timezone, timedelta
    tz_uz    = timezone(timedelta(hours=5))
    now      = datetime.now(tz_uz)
    # O'tgan haftaning boshlanish/tugash sanalari (dushanba-yakshanba)
    today    = now.date()
    # Haftaning boshlanishi (o'tgan dushanba)
    days_since_monday = today.weekday()  # dushanba=0
    ...
    for uid_str, udata in users.items():
        try:
            ...
            # Jon boshlanish qiymati (haftada 7 kun, har bir kun o'zgaradi)
            jon_end   = udata.get("jon", 100)
            # Jon boshlanishini taxminiy hisoblash (7 kunlik o'rtacha)
            jon_start = max(0, min(100, round(jon_end - (jon_end - 50) * 0.1)))

            # Har bir odatning haftalik statistikasi
            total_possible = 0
            total_done_w   = 0
            habit_scores   = []
            best_streak    = 0
            ...
            for h in habits:
                ...
                habit_scores.append((h["name"], score, done, poss))
            ...
            # Ball hisoblash (taxminiy: bajarilgan odatlar * 5)
            balls_earned = total_done_w * 5

            # Eng yaxshi va eng yomon odat
    ...
    print(f"[weekly_report] Yuborildi.")
...
def create_challenge(creator_uid, name, duration):
    ch_id = "ch_" + _uuid.uuid4().hex[:8]
    ch = {
        "_id":        ch_id,
        "name":       name,
        "creator":    creator_uid,
        "members":    [creator_uid],
        "duration":   duration,
        "start_date": None,   # Birinchi a'zo qo'shilganda boshlanadi
        "end_date":   None,
        "progress":   {str(creator_uid): []},
        "active":     False,
        "finished":   False,
    }
    challenges_col.insert_one(ch)
    return ch_id

def get_challenge(ch_id):
    return challenges_col.find_one({"_id": ch_id})

def save_challenge(ch):
    challenges_col.update_one({"_id": ch["_id"]}, {"$set": ch}, upsert=True)

def get_user_challenges(uid):
    return list(challenges_col.find({"members": uid, "finished": False}))

def challenge_status_text(ch, viewer_uid=None):
    """Challenge holati matni"""
    from datetime import timezone, timedelta
    tz_uz    = timezone(timedelta(hours=5))
    ...
    if active and start:
        ...
        t = date.today()
        day_num = (t - s).days + 1
        header  = f"👥 *{name}* — {day_num}/{duration}-kun"
    else:
        header = f"👥 *{name}* — kutilmoqda"
    ...
    for m_uid in members:
        u_data   = load_user(m_uid)
        m_name   = u_data.get("name", "?")
        days_done = progress.get(str(m_uid), [])
        streak   = 0
        # Streak hisoblash
        if active and start:
            ...
            for _ in range(duration):
                if check.isoformat() in days_done:
                    streak += 1
                    check -= td(days=1)
                else:
                    break
        ...
        rows.append((len(days_done), m_name, tick, streak, m_uid))

    rows.sort(key=lambda x: x[0], reverse=True)
    for i, (done_cnt, m_name, tick, streak, m_uid) in enumerate(rows):
        medal  = medals[i] if i < len(medals) else "▫️"
        is_me  = " _(siz)_" if m_uid == viewer_uid else ""
        streak_txt = f" 🔥 {streak}" if streak > 0 else ""
        text += f"{medal} {tick} {m_name}{streak_txt}{is_me}\n"

    if active:
        total_done = sum(1 for r in rows if r[2] == "✅")
        pct = round(total_done / len(members) * 100) if members else 0
        text += f"\n*📊 Guruh natijasi:* {pct}%"
    else:
        text += f"\n*👥 A'zolar:* {len(members)}/5"
        text += f"\n*⏳ Boshlash uchun kamida 2 kishi kerak*"
    return text

def challenge_finish(ch):
    """Challenge yakunlash va ball taqsimlash"""
    members  = ch.get("members", [])
    progress = ch.get("progress", {})
    rows = []
    for m_uid in members:
        done = len(progress.get(str(m_uid), []))
        rows.append((done, m_uid))
    rows.sort(reverse=True)
    prizes = {0: 100, 1: 50, 2: 25}
    duration = ch.get("duration", 7)
    medals_txt = ["🥇","🥈","🥉"]
    result_text  = f"🏆 *{ch['name']}* yakunlandi!\n\n"
    for i, (done, m_uid) in enumerate(rows):
        u_d = load_user(m_uid)
        name = u_d.get("name", "?")
        pct  = round(done / duration * 100) if duration else 0
        medal = medals_txt[i] if i < len(medals_txt) else f"{i+1}."
        prize = prizes.get(i, 0)
        result_text += f"{medal} {name} — {done}/{duration} kun ({pct}%)"
        if prize:
            result_text += f" +{prize} ball"
        result_text += "\n"
        # Ball berish
        if prize:
            u_d["points"] = u_d.get("points", 0) + prize
            save_user(m_uid, u_d)
    result_text += "\n*🎁 Ballar hisobingizga qo'shildi!*"
    ch["finished"] = True
    ch["active"]   = False
    save_challenge(ch)
    for m_uid in members:
        try:
            bot.send_message(m_uid, result_text, parse_mode="Markdown")
        except Exception:
            pass
...
def callback_handler(call):
    ...
    if cdata == "menu_settings":
        ...
        kb_set.add(InlineKeyboardButton(T(uid, "btn_change_info"),        callback_data="settings_info"))
        kb_set.add(InlineKeyboardButton("⚙️ Odat sozlamalari",           callback_data="settings_habits_time"))
        kb_set.add(InlineKeyboardButton("👥 Challengelarim",             callback_data="my_challenges"))
        kb_set.add(InlineKeyboardButton("💬 Dasturchiga habar",           callback_data="settings_contact_dev"))
        kb_set.add(cBtn(T(uid, "btn_home"), "menu_main", "primary"))
        sent = bot.send_message(uid, T(uid, "settings_title"), parse_mode="Markdown", reply_markup=kb_set)
        u["main_msg_id"] = sent.message_id
    ...
    if cdata == "menu_add":
        ...
        kb_type.add(InlineKeyboardButton("👥 Challenge", callback_data="habit_type_challenge"))
        kb_type.add(InlineKeyboardButton(T(uid, "btn_cancel"), callback_data="cancel"))
        sent = bot.send_message(
            uid,
            "📋 *Odat turini tanlang:*\n\n"
            "📌 *Oddiy* — kuniga 1 marta\n"
            "🔁 *Takrorlanuvchi* — kuniga bir necha marta\n"
            "👥 *Challenge* — do'stlar bilan birga bajaring!",
    ...
    if cdata == "habit_type_challenge":
        bot.answer_callback_query(call.id)
        bot.delete_message(uid, call.message.message_id)
        u["state"] = "challenge_waiting_name"
        cancel_kb = InlineKeyboardMarkup()
        cancel_kb.add(InlineKeyboardButton(T(uid, "btn_cancel"), callback_data="cancel_to_main"))
        sent = bot.send_message(
            uid,
            "👥 *Challenge yaratish*\n\n"
            "Challenge nomini yozing:\n"
            "_Masalan: Har kuni yugurish, Kitob o'qish_",
            parse_mode="Markdown", reply_markup=cancel_kb
        )
        u["temp_msg_id"] = sent.message_id
        save_user(uid, u)
    ...
    if cdata.startswith("challenge_duration_"):
        duration = int(cdata[len("challenge_duration_"):])
        bot.answer_callback_query(call.id)
        bot.delete_message(uid, call.message.message_id)
        ch_name = u.get("temp_challenge_name", "Challenge")
        ch_id   = create_challenge(uid, ch_name, duration)
        bot_info = bot.get_me()
        invite_link = f"https://t.me/{bot_info.username}?start=join_{ch_id}"
        u.pop("temp_challenge_name", None)
        u["state"] = None
        save_user(uid, u)
        text  = f"✅ *{ch_name}* challenge yaratildi!\n\n"
        text += f"*⏳ Davomiyligi:* {duration} kun\n"
        text += f"*👥 A'zolar:* 1/5\n\n"
        text += "▬" * 16 + "\n"
        text += "*🔗 Invite link:*\n"
        text += f"`{invite_link}`\n\n"
        text += "_Do'stlaringizga yuboring — ular qo'shilsin!_\n"
        text += "_Kamida 2 kishi bo'lganda challenge boshlanadi._"
        kb_ch = InlineKeyboardMarkup()
        kb_ch.add(InlineKeyboardButton("📋 Havolani nusxalash", switch_inline_query=invite_link))
        kb_ch.add(InlineKeyboardButton("👥 Challengelarim", callback_data="my_challenges"))
        kb_ch.add(cBtn(T(uid, "btn_home"), "menu_main", "primary"))
        sent = bot.send_message(uid, text, parse_mode="Markdown", reply_markup=kb_ch)
        u["main_msg_id"] = sent.message_id
        save_user(uid, u)
        return
    ...
    if cdata == "my_challenges":
        bot.answer_callback_query(call.id)
        bot.delete_message(uid, call.message.message_id)
        # Pending join tekshirish
        pj = u.pop("pending_join_challenge", None)
        if pj:
            ch = get_challenge(pj)
            if ch and not ch.get("finished") and uid not in ch["members"] and len(ch["members"]) < 5:
                ch["members"].append(uid)
                ch["progress"][str(uid)] = []
                if len(ch["members"]) >= 2 and not ch.get("active"):
                    from datetime import timezone, timedelta, date
                    ...
                    try:
                        bot.send_message(ch["creator"],
                            f"👥 *{ch['name']}* ga yangi a'zo qo'shildi!\n"
                            f"A'zolar: {len(ch['members'])}/5",
                            parse_mode="Markdown")
                    except: pass
            save_user(uid, u)
        challenges = get_user_challenges(uid)
        if not challenges:
            kb_no = InlineKeyboardMarkup()
            kb_no.add(InlineKeyboardButton("➕ Challenge yaratish", callback_data="menu_add"))
            kb_no.add(cBtn(T(uid, "btn_home"), "menu_main", "primary"))
            sent_nc = bot.send_message(uid,
                "👥 *Challengelar*\n\nHali challenge yo'q.\n"
                "Yangi challenge yarating yoki do'stingizning invite linkini bosing!",
                parse_mode="Markdown", reply_markup=kb_no)
            u["main_msg_id"] = sent_nc.message_id
            save_user(uid, u)
            return
        kb_chl = InlineKeyboardMarkup()
        for ch in challenges:
            status = "🟢" if ch.get("active") else "⏳"
            kb_chl.add(InlineKeyboardButton(
                f"{status} {ch['name']} ({ch['duration']} kun)",
                callback_data=f"challenge_view_{ch['_id']}"
            ))
        kb_chl.add(cBtn(T(uid, "btn_home"), "menu_main", "primary"))
        sent_cl = bot.send_message(uid, "👥 *Challengelarim*", parse_mode="Markdown", reply_markup=kb_chl)
        u["main_msg_id"] = sent_cl.message_id
        save_user(uid, u)
        return

    if cdata.startswith("challenge_view_"):
        ch_id = cdata[len("challenge_view_"):]
        bot.answer_callback_query(call.id)
        bot.delete_message(uid, call.message.message_id)
        ch = get_challenge(ch_id)
        if not ch:
        ...
        if ch.get("active") and not done_today:
            kb_cv.add(cBtn("✅ Bugun bajardim!", f"challenge_done_{ch_id}", "primary"))
        elif ch.get("active") and done_today:
            kb_cv.add(InlineKeyboardButton("✅ Bugun bajarildi", callback_data="noop"))
        kb_cv.row(
            cBtn("⬅️ Orqaga", "my_challenges", "primary"),
            cBtn(T(uid, "btn_home"), "menu_main", "primary")
        )
        sent_cv = bot.send_message(uid, text, parse_mode="Markdown", reply_markup=kb_cv)
        u["main_msg_id"] = sent_cv.message_id
        save_user(uid, u)
        return

    if cdata.startswith("challenge_done_"):
        ch_id = cdata[len("challenge_done_"):]
        ...
        uid_key = str(uid)
        if today_s not in prog.get(uid_key, []):
            prog.setdefault(uid_key, []).append(today_s)
            ch["progress"] = prog
            # Challenge tugadimi?
            end_d = ch.get("end_date")
            if end_d and date.today().isoformat() >= end_d:
                challenge_finish(ch)
            else:
                save_challenge(ch)
        # Ko'rinishni yangilash
        done_today = today_s in ch["progress"].get(uid_key, [])
        text = challenge_status_text(ch, viewer_uid=uid)
        kb_cv = InlineKeyboardMarkup()
        kb_cv.add(InlineKeyboardButton("✅ Bugun bajarildi", callback_data="noop"))
        kb_cv.row(
        ...
        try:
            bot.edit_message_text(text, uid, call.message.message_id,
                parse_mode="Markdown", reply_markup=kb_cv)
        except: pass
        return
...
def scheduler_loop():
    ...
    # Har kuni 20:00 (UTC+5 = 15:00 UTC) da challenge eslatma
    schedule.every().day.at("15:00").do(send_challenge_reminders).tag("challenge_reminder")
    while True:
