...
def handle_text(msg):
    ...
    if state and state.startswith("admin_waiting_reply_") and uid == ADMIN_ID:
        ...
        try:
            ...
            bot.send_message(uid, "✅ Javob yuborildi!", reply_markup=admin_menu())
        except Exception as e:
...
def callback_handler(call):
    ...
    if cdata == "dismiss_ball_notif":
        ...
        try:
            sent_admin_notif = bot.send_message(
                ADMIN_ID,
                f"✅ *{user_name}* (ID: `{uid}`) ballni olganini tasdiqladi!",
                parse_mode="Markdown"
            )
            # Adminga kelgan xabar 5 soniyada o'chsin
            def del_admin_confirm(mid):
                time.sleep(5)
                try: bot.delete_message(ADMIN_ID, mid)
                except: pass
            threading.Thread(target=del_admin_confirm, args=(sent_admin_notif.message_id,), daemon=True).start()
        except Exception:
            pass
        # Foydalanuvchidagi xabarni o'chirish
        def del_ball_notif(chat_id, msg_id):
            time.sleep(5)
            try: bot.delete_message(chat_id, msg_id)
            except: pass
        threading.Thread(target=del_ball_notif, args=(uid, call.message.message_id), daemon=True).start()
        return
        ...
        for h in u.get("habits", []):
            if h["id"] == habit_id:
                ...
                if hab_type == "repeat" and rep_count > 1:
                    ...
                    if fully_done:
                        # Bekor qilish: birini ayir
                        done_today_count = max(0, done_today_count - 1)
                        ...
                        if done_today_count < rep_count:
                            h["last_done"] = None
                        save_user(uid, u)
                        bot.answer_callback_query(call.id, f"↩️ {done_today_count}/{rep_count}")
                    else:
                        # Bir bosish = +1
                        done_today_count += 1
                        h["done_today_count"] = done_today_count
                        ...
                        if done_today_count >= rep_count:
                            ...
                            sent_msg = bot.send_message(
                                uid,
                                T(uid, "done_ok", name=h["name"]) + f" *+5 ⭐ ball* ✦ {rep_count}/{rep_count}",
                                parse_mode="Markdown"
                            )
                        else:
                            save_user(uid, u)
                            ...
                            sent_msg = bot.send_message(
                                uid,
                                f"✅ *{h['name']}* — *{done_today_count}/{rep_count}* bajarildi! *+5 ⭐ ball*",
                                parse_mode="Markdown"
                            )
