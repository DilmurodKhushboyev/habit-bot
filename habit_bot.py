...
def handle_text(msg):
    ...
    if state == "waiting_habit_time":
        ...
        if hab_type == "repeat":
            ...
            if not u.get("temp_habit"):
                ...
                send_main_menu(uid)
                return
            # Repeat: necha marta so'rash
            collected = u["temp_habit"].get("times_collected", [])
            collected.append(time_text)
            u["temp_habit"]["times_collected"] = collected
            # Nechta vaqt kerakligini bilish uchun repeat_count so'rash
            if not u["temp_habit"].get("repeat_count_asked"):
                u["temp_habit"]["repeat_count_asked"] = True
                save_user(uid, u)
                cancel_kb = InlineKeyboardMarkup()
                cancel_kb.add(InlineKeyboardButton(T(uid, "btn_cancel"), callback_data="cancel"))
                # Yana bir vaqt so'rash yoki yakunlash
                kb_rc = InlineKeyboardMarkup()
                kb_rc.add(InlineKeyboardButton(f"✅ {len(collected)} ta — Shu bilan tugatish", callback_data="repeat_done"))
                kb_rc.add(InlineKeyboardButton("➕ Yana bir vaqt qo'shish", callback_data="repeat_add_more"))
                kb_rc.add(InlineKeyboardButton(T(uid, "btn_cancel"), callback_data="cancel"))
                u["state"] = "waiting_repeat_more"
                sent = bot.send_message(
                    uid,
                    f"✅ Vaqt qo'shildi: *{time_text}*\n\nJami: *{len(collected)} ta* vaqt\n\nYana qo'shasizmi?",
                    parse_mode="Markdown", reply_markup=kb_rc
                )
                u["temp_msg_id"] = sent.message_id
                save_user(uid, u)
            else:
    ...
    if state == "admin_waiting_points_id" and uid == ADMIN_ID:
        ...
        save_user(uid, u)
        cancel_kb = InlineKeyboardMarkup()
        cancel_kb.add(InlineKeyboardButton("❌ Bekor qilish", callback_data="admin_cancel"))
        target_u = load_user(target_id)
        sent = bot.send_message(
            uid,
