...
const user      = tg?.initDataUnsafe?.user || { id: urlUserId || 0, first_name: 'Test' };
const initData  = tg?.initData || '';
const userId    = user.id || urlUserId || 0;
...
async function startApp() {
  if (!userId || userId == 0 || userId === '0') {
    document.getElementById('today-content').innerHTML =
      '<div class="empty-state"><div class="icon">⚠️</div>Foydalanuvchi aniqlanmadi.<br><small>Botdan qayta oching.</small></div>';
    return;
  }
  loadToday();
}
