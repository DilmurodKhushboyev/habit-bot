    ...
    for mid in [old_admin_msg, old_main_msg]:
        if mid:
            try:
                ...
                pass
    sent_admin = bot.send_message(uid, "🛠 *Admin panel | Maxfiy qism*", parse_mode="Markdown", reply_markup=admin_menu())
    u = load_user(uid)
...
def handle_text(msg):
    ...
    if state in ("admin_bc_private", "admin_bc_groups", "admin_bc_all") and uid == ADMIN_ID:
        ...
        bot.send_message(uid, "🛠 *Admin panel | Maxfiy qism*", parse_mode="Markdown", reply_markup=admin_menu())
        return
        ...
        try:
            ...
            def delete_confirm_and_menu(chat_id, message_id):
                ...
                try:
                    ...
                    pass
                bot.send_message(chat_id, "🛠 *Admin panel | Maxfiy qism*", parse_mode="Markdown", reply_markup=admin_menu())
    ...
    if state == "waiting_habit_name":
        ...
        if hab_type == "repeat" and rep_count > 1:
            # Birinchi vaqtni so'rash
            u["state"] = "waiting_habit_repeat_times"
            prompt = (
                f"⏰ *{text}* odati uchun *1/{rep_count}* eslatma vaqtini kiriting:\n\n"
                f"_Masalan: 06:00_"
            )
        else:
    ...
    if state == "bozor_waiting_subtract":
        ...
        sent2 = bot.send_message(uid, f"✏️ *Ballarni tahrirlash*\n\n⭐ Joriy balingiz: *{new_balls} ball*", parse_mode="Markdown", reply_markup=kb_edit)
        u2 = load_user(uid)
        u2["main_msg_id"] = sent2.message_id
        save_user(uid, u2)
        return
...
def callback_handler(call):
    ...
    if cdata == "menu_bozor":
        ...
        sent = bot.send_message(
            ...
            "✏️ *Ballarni tahrirlash* — o'z ballaringizni sozlash",
    ...
    if cdata == "bozor_reset_confirm":
        ...
        kb_conf.row(
            ...
            InlineKeyboardButton("❌ Yo'q",               callback_data="bozor_edit")
        )
        sent = bot.send_message(
            uid,
    ...
    if cdata == "bozor_reset_do":
        ...
        sent2 = bot.send_message(uid, "✏️ *Ballarni tahrirlash*\n\n⭐ Joriy balingiz: *0 ball*", parse_mode="Markdown", reply_markup=kb_edit)
        u2 = load_user(uid)
        u2["main_msg_id"] = sent2.message_id
        save_user(uid, u2)
        return
    ...
    if cdata == "bozor_subtract":
        ...
        cancel_kb = InlineKeyboardMarkup()
        cancel_kb.row(
            InlineKeyboardButton("⬅️ Orqaga", callback_data="bozor_edit"),
            InlineKeyboardButton(T(uid, "btn_home"), callback_data="menu_main")
        )
        sent = bot.send_message(
            uid,
