...
def menu2_dict(uid=None):
    ...
    if uid:
        ...
        for g_info in groups:
            g_id   = g_info.get("id", "")
            g_name = g_info.get("name", "Guruh")
            # Bugun bajardimi? — yashil (success) yoki rangsiz
            g_data  = load_group(g_id)
            i_done  = False
            if g_data:
                done_today = g_data.get("done_today", {})
                if g_data.get("done_date") != today:
                    done_today = {}
                i_done = done_today.get(str(uid), False)
            style = None if i_done else "success"
            label = f"☑️ {g_name}" if i_done else f"👥 {g_name}"
            row = {"text": label, "callback_data": f"group_view_{g_id}"}
            if style:
                row["style"] = style
            rows.append([row])
    rows.append([
    ...
    rows.append([
        {"text": "🏠 1-menyu",           "callback_data": "menu_main",      "style": "primary"},
    ])
    return {"inline_keyboard": rows}
...
def callback_handler(call):
    ...
    if cdata.startswith("group_view_"):
        ...
        members    = g.get("members", [])
        today      = today_uz5()
        done_today = g.get("done_today", {})        # {uid_str: True/False}
        ...
        for mid in members:
            mu     = load_user(mid)
            m_name = mu.get("name", "?")
            m_done = done_today.get(str(mid), False)
            if m_done: status = "✅"
            else:      status = "⬜"
            you = " *(siz)*" if mid == uid else ""
            members_text += f"{status} {m_name}{you}\n"
        text  = f"👥 *{g['name']}*\n"
        text += "━" * 16 + "\n"
        text += f"📌 Odat: *{g.get('habit_name','—')}*\n"
        text += f"⏰ Vaqt: *{g.get('habit_time','vaqtsiz')}*\n"
        text += f"🔥 Streak: *{g.get('streak',0)} kun*\n"
        text += f"👤 A'zolar: *{len(members)} ta*\n\n"
        text += "━" * 16 + "\n"
        text += f"*Bugungi progress:* {done_count}/{len(members)}\n\n"
        text += members_text
        kb_g = InlineKeyboardMarkup()
        if not i_done:
            kb_g.add(cBtn("✅ Men ham bajardim!", f"group_done_{g_id}", "success"))
        else:
            kb_g.add(InlineKeyboardButton("☑️ Bajarildi", callback_data="noop"))
        kb_g.add(InlineKeyboardButton("🔗 Invite link", callback_data=f"group_invite_{g_id}"))
        if g.get("admin_id") == uid:
            kb_g.add(cBtn("🗑 Guruhni o'chirish", f"group_delete_confirm_{g_id}", "danger"))
        else:
            kb_g.add(cBtn("🚪 Guruhdan chiqish", f"group_leave_{g_id}", "danger"))
        kb_g.add(cBtn("⬅️ Orqaga", "menu2_open", "primary"))
        sent = bot.send_message(uid, text, parse_mode="Markdown", reply_markup=kb_g)
        u["main_msg_id"] = sent.message_id
        save_user(uid, u)
        return

    if cdata.startswith("group_done_"):
        g_id = cdata[len("group_done_"):]
        ...
        members    = g.get("members", [])
        today      = today_uz5()
        # Bugungi sana o'zgargan bo'lsa — tozalash
        if g.get("done_date") != today:
            g["done_today"] = {}
            g["done_date"]  = today
        done_today = g.get("done_today", {})
        if done_today.get(str(uid)):
            # Allaqachon bajarilgan
            return
        done_today[str(uid)] = True
        ...
        if str(uid) not in done_log:
            done_log[str(uid)] = {}
        done_log[str(uid)][today] = True
        g["member_done_log"] = done_log
        # member_streaks: {uid_str: int}
        ...
        # Yangilangan group_view ni ko'rsatish
        done_today2 = g.get("done_today", {})
        done_count2 = sum(1 for v in done_today2.values() if v)
        members_text = ""
        for mid in members:
            mu     = load_user(mid)
            m_name = mu.get("name", "?")
            m_done = done_today2.get(str(mid), False)
        ...
        text += "━" * 16 + "\n"
        if all_done:
            text += f"🎉 *Bugun barchangiz bajardingiz!*\n\n"
        else:
            text += f"*Bugungi progress:* {done_count2}/{len(members)}\n\n"
        text += members_text
        kb_g = InlineKeyboardMarkup()
        kb_g.add(InlineKeyboardButton("☑️ Bajarildi", callback_data="noop"))
        kb_g.add(InlineKeyboardButton("🔗 Invite link", callback_data=f"group_invite_{g_id}"))
    ...
    if cdata.startswith("group_approve_"):
        ...
        sent_adm = bot.send_message(uid,
            f"✅ *{ju.get('name','Foydalanuvchi')}* guruhga qo'shildi!\n"
            f"👥 *{g['name']}*",
            parse_mode="Markdown"
        )
        def del_adm(chat_id, mid):
            time.sleep(5)
            try: bot.delete_message(chat_id, mid)
            except: pass
        threading.Thread(target=del_adm, args=(uid, sent_adm.message_id), daemon=True).start()
    ...
    if cdata.startswith("group_reject_"):
        ...
        try:
            sent_rej = bot.send_message(joiner_id,
                f"❌ *{g['name'] if g else 'Guruh'}* guruhiga qo'shilish so'rovingiz rad etildi.",
                parse_mode="Markdown"
            )
            def del_rej(chat_id, mid):
                time.sleep(5)
                try: bot.delete_message(chat_id, mid)
                except: pass
            threading.Thread(target=del_rej, args=(joiner_id, sent_rej.message_id), daemon=True).start()
        except: pass
        return
...
def _save_new_habit(uid, u):
    ...
    def del_and_back(chat_id, mid):
        ...
        menu_dict = {
            "inline_keyboard": [
                [{"text": "📌 Oddiy", "callback_data": "habit_type_simple", "style": "primary"},
                 {"text": "🔁 Takrorlanuvchi", "callback_data": "habit_type_repeat", "style": "primary"}],
...
def handle_text(msg):
    ...
    # ── Broadcast matn (admin) ──
    if state in ("admin_bc_private", "admin_bc_groups", "admin_bc_all") and uid == ADMIN_ID:
        # Admin xabarini o'chirmasdan oldin message_id ni saqlaymiz (copy_message uchun)
        bc_msg_id = msg.message_id
        users      = load_all_users()
        sent_count = 0
        failed     = 0
        failed_ids = []
        # Yuborish jarayonida admin ga progress xabar
        prog_msg = bot.send_message(uid, f"⏳ Yuborilmoqda... (0/{len(users)} ta)")
        for target_uid_str in users:
            try:
                target_uid_int = int(target_uid_str)
            except (ValueError, TypeError):
                continue  # raqam bo'lmagan ID larni o'tkazib yuborish
            if target_uid_int == ADMIN_ID:
                continue
            try:
                # Admin qanday matn yuborganini saqlash: Markdown bo'lsa Markdown, bo'lmasa oddiy
                try:
                    bot.copy_message(target_uid_int, uid, bc_msg_id)
                except Exception:
                    bot.send_message(target_uid_int, text)
                sent_count += 1
                time.sleep(0.04)
            except Exception as e:
                err_str = str(e)
        ...
        try: bot.delete_message(uid, prog_msg.message_id)
        except: pass
        u["state"] = None
        if failed_ids:
        ...
        sent_r = bot.send_message(uid, result, reply_markup=kb_bc)
        # 5 soniyadan keyin natija xabarini o'chirib admin menyuga qaytish
        ...
        threading.Thread(target=del_bc_result, args=(uid, sent_r.message_id), daemon=True).start()
        return

    # ── Telefon raqamni matn sifatida yangilash ──
    if state == "updating_phone":
        import re as _re
        phone_text  = text.strip()
        phone_clean = _re.sub(r"[^\d+]", "", phone_text)
        if len(phone_clean) >= 9:
            try: bot.delete_message(uid, msg.message_id)
            except: pass
            info_msg_id = u.pop("info_msg_id", None)
            ...
            u["state"] = None
            save_user(uid, u)
            sent_ok = bot.send_message(uid, "✅ Telefon raqam yangilandi!", reply_markup=ReplyKeyboardRemove())
            def del_ok_phone(chat_id, mid):
                time.sleep(2)
                try: bot.delete_message(chat_id, mid)
                except: pass
                send_main_menu(chat_id)
            threading.Thread(target=del_ok_phone, args=(uid, sent_ok.message_id), daemon=True).start()
        else:
            bot.send_message(uid, "⚠️ Noto'g'ri raqam. Masalan: *+998901234567*", parse_mode="Markdown")
        return

    # ── Repeat odat: tugmani kutish holati ──
    if state == "waiting_repeat_more":
        # Bu state faqat tugma bilan boshqariladi, matn kelmaydi
        try: bot.delete_message(uid, msg.message_id)
        except: pass
        return

    # ── Noaniq xabar (faqat state yo'q bo'lganda) ──
    if u.get("phone") and not state:
        bot.send_message(uid, T(uid, "invalid_msg"), parse_mode="Markdown")
