...
def cmd_start(msg):
    ...
    if start_param.startswith("join_"):
        ...
        if ch and not ch.get("finished") and uid not in ch.get("members", []) and len(ch.get("members", [])) < 5:
            u_check = load_user(uid)
            u_check.setdefault("pending_join_challenge", ch_id)
    ...
    if cdata == "cancel":
        ...
        if old_msg_id:
            try:
                ...
                pass
        # Odat yaratish menyusiga qaytish
        bot.answer_callback_query(call.id)
        kb_type = InlineKeyboardMarkup()
        kb_type.row(
            InlineKeyboardButton("📌 Oddiy",          callback_data="habit_type_simple"),
        ...
        kb_type.add(InlineKeyboardButton(T(uid, "btn_cancel"), callback_data="cancel_to_main"))
