...
LANGS = {
    "uz": {
        ...
        "change_info_text":  "📝 *Ma'lumotlarni o'zgartirish*\n\nQuyidagi tugma orqali yangi telefon raqamingizni yuboring:",
    ...
    "en": {
        ...
        "change_info_text":  "📝 *Change Profile*\n\nSend your new phone number using the button below:",
    ...
    "ru": {
        ...
        "change_info_text":  "📝 *Изменить профиль*\n\nОтправьте новый номер телефона кнопкой ниже:",
    ...
    "tr": {
        ...
        "change_info_text":  "📝 *Profili Düzenle*\n\nAşağıdaki butonu kullanarak yeni telefon numaranızı gönderin:",
...
def handle_contact(msg):
    ...
    if state == "updating_info":
        ...
        def del_ok(chat_id, mid):
            time.sleep(2)
            try: bot.delete_message(chat_id, mid)
            except: pass
        threading.Thread(target=del_ok, args=(uid, sent_ok.message_id), daemon=True).start()
        send_main_menu(uid)
        return
...
def callback_handler(call):
    ...
    if cdata == "settings_info":
        bot.answer_callback_query(call.id)
        bot.delete_message(uid, call.message.message_id)
        u["state"] = "updating_info"
        save_user(uid, u)
        kb_info = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
        kb_info.add(KeyboardButton("📱 Yangi raqam yuborish", request_contact=True))
...
def daily_reset():
    """Har kuni 00:00 da barcha jadvallarni qayta yuklash (bugun bajarilgan odatlar uchun ham)"""
    print("[daily_reset] Barcha jadvallar qayta yuklanmoqda...")
    schedule.clear()
    load_all_schedules()
    # O'zini qayta rejalashtirish
    schedule.every().day.at("19:00").do(daily_reset).tag("daily_reset")  # 00:00 UTC+5 = 19:00 UTC
