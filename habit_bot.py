...
def cmd_start(msg):
    ...
    # Oldingi barcha xabarlarni o'chirish (/start o'zi bundan mustasno)
    u_prev = load_user(uid)
    to_delete = list(u_prev.get("start_msg_ids", []))
    old_main  = u_prev.get("main_msg_id")
    if old_main and old_main not in to_delete:
        to_delete.append(old_main)
    # /start buyrug'ining o'zini ham o'chirish (eski /start xabari)
    to_delete.append(msg.message_id)
    # Ro'yxatni tozalaymiz
    u_prev["start_msg_ids"] = []
    u_prev["main_msg_id"]   = None
    save_user(uid, u_prev)
    def delete_prev_msgs(chat_id, ids):
        for mid in ids:
            try: bot.delete_message(chat_id, mid)
            except: pass
    threading.Thread(target=delete_prev_msgs, args=(uid, to_delete), daemon=True).start()
