...
def main_menu(uid=None):
    ...
    # Avval harakatlar tugmalari (yuqorida)
    kb.row(
    ...
    # Keyin odatlar tugmalari (pastda)
    if uid:
        u      = load_user(uid)
...
def build_main_text(uid):
    ...
    habits = u.get("habits", [])
    if not habits:
        return "Quyidagi tugmalardan foydalaning 👇"
...
def handle_text(msg):
    ...
    if state == "waiting_habit_time":
        try:
            ...
            sent_err = bot.send_message(uid, "❌ Noto'g'ri format. Iltimos HH:MM formatida yozing.\n_Masalan: 08:00_", parse_mode="Markdown")
...
def show_stats(uid, page=1):
    ...
    text  = f"📊 *Statistika* — {page}/{total_pages} sahifa\n"
    text += "▬" * 16 + "\n"
    text += f"⭐ *Jami ball: {total_points} ball*\n"
    text += f"🎖 Daraja: {rank}\n"
    text += "▬" * 16 + "\n\n"
    ...
    nav_row  = []
    if page > 1:
        nav_row.append(InlineKeyboardButton(f"⬅️ {page - 1}-sahifa", callback_data=f"stats_page_{page - 1}"))
    if page < total_pages:
        nav_row.append(InlineKeyboardButton(f"{page + 1}-sahifa ➡️", callback_data=f"stats_page_{page + 1}"))
    if nav_row:
        kb_stats.row(*nav_row)
    kb_stats.add(InlineKeyboardButton("🏠 Asosiy menyu", callback_data="menu_main"))
...
def show_rating(uid):
    ...
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("🏠 Asosiy menyu", callback_data="menu_main"))
    u    = load_user(uid)
...
def delete_habit_menu(uid):
    ...
    habits = u.get("habits", [])
    if not habits:
        bot.send_message(uid, "📭 O'chirish uchun odat yo'q.", reply_markup=main_menu(uid))
        return
    kb = InlineKeyboardMarkup()
    for h in habits:
...
def callback_handler(call):
    ...
    if cdata == "menu_add":
        ...
        if len(u.get("habits", [])) >= 15:
            bot.answer_callback_query(call.id)
            sent_limit = bot.send_message(uid, "⚠️ Odatlarni yaratish me'yori 15 ta!")
    ...
    if cdata.startswith("confirm_delete_"):
        ...
        try:
            ...
            pass
        sent_del = bot.send_message(uid, f"🗑 *{habit_name}* o'chirildi.", parse_mode="Markdown")
    ...
    # --- toggle_ (asosiy menyudan odat bajarildi/bekor) ---
        ...
        for h in u.get("habits", []):
            if h["id"] == habit_id:
                if h.get("last_done") == today:
                    ...
                    save_user(uid, u)
                    bot.answer_callback_query(call.id)
                    # "Odat bajarildi ❌" xabarini yuborib 3 soniyada o'chirish
                    sent_msg = bot.send_message(uid, f"❌ *{h['name']}* — bekor qilindi!", parse_mode="Markdown")
                    ...
                    bot.answer_callback_query(call.id)
                    # "Odat bajarildi ✅" xabarini yuborib 3 soniyada o'chirish
                    sent_msg = bot.send_message(uid, f"✅ *{h['name']}* — bajarildi!", parse_mode="Markdown")
                    ...
                    # Barcha odatlar bajarildi tekshiruvi
                    all_done = all(hh.get("last_done") == today for hh in u.get("habits", []))
                    if all_done:
                        sent_congrats = bot.send_message(uid, "Bugungi barcha odatni bajardingiz, tabriklaymiz 🎉")
                        def del_congrats(chat_id, msg_id):
                            time.sleep(5)
                            try:
                                bot.delete_message(chat_id, msg_id)
                            except Exception:
                                pass
                        threading.Thread(target=del_congrats, args=(uid, sent_congrats.message_id), daemon=True).start()
                # Menyuni matn bilan birga yangilash
