...
try:
    ...
    def api_ai_advice(uid):
        ...
        try:
            ...
            if resp.status_code != 200:
                _err_body = ""
                try:
                    _err_body = resp.json().get("error", {}).get("message", resp.text[:200])
                except Exception:
                    _err_body = resp.text[:200]
                print(f"[ai_advice] Anthropic {resp.status_code}: {_err_body}")
                return jsonify({"ok": False, "error": f"AI xato ({resp.status_code}): {_err_body}"}), resp.status_code
            result = resp.json()
            advice = result["content"][0]["text"].strip()
        except _req.exceptions.ConnectionError as e:
            print(f"[ai_advice] connection error: {e}")
            return jsonify({"ok": False, "error": "AI serveriga ulanib bo'lmadi. Internet yoki firewall muammosi."}), 502
        except _req.exceptions.Timeout:
            print("[ai_advice] timeout")
            return jsonify({"ok": False, "error": "AI server javob bermadi (timeout). Qayta urinib ko'ring."}), 504
        except Exception as e:
            print(f"[ai_advice] xato: {type(e).__name__}: {e}")
            return jsonify({"ok": False, "error": f"AI xatosi: {type(e).__name__}"}), 502
        # Limitni yangilash
    ...
    def setup_webhook():
        ...
        try:
            ...
            # Bot bio va tavsifini o'rnatish
            try:
                _bio = 'Necha marta "ertadan boshlayman" dedingiz?\n\n\U0001f3d4\ufe0f Balki bugun boshlarmiz?\n\nMurojaat uchun: @Dilmurod_Qoshboyev'
                bot.set_my_short_description(_bio)
                _desc = ("\U0001f4c8 Har kuni 1% ga yaxshilansangiz, yil oxirida "
                         "37 barobar kuchliroq bo\u2019lasiz.\n\n"
                         "Maqsad natijani belgilaydi, \U0001f331 tizim "
                         "(jarayon) esa unga olib boradi.")
                bot.set_my_description(_desc)
            except Exception as _be:
                print(f"[setup] bio/desc: {_be}")
