...
def main_menu(uid=None, page=1):
    ...
    if uid:
        ...
        # Bajarilmaganlar avval, bajarilganlar oxirga
        not_done = []
        done_list = []
        for h in page_habits:
            ...
            if hab_type == "repeat" and rep_count > 1:
                ...
                if done_today >= rep_count:
                    ...
                    done_list.append((label, h['id']))
                else:
                    label = f"✅ {h['name']} {done_today}/{rep_count}"
                    not_done.append((label, h['id']))
            else:
                ...
                if is_done:
                    ...
                    label = f"✅ {h['name']}"
                    not_done.append((label, h['id']))
        for label, hid in not_done + done_list:
            kb.add(InlineKeyboardButton(label, callback_data=f"toggle_{hid}"))
        if total_pages > 1:
            nav = []
            if page > 1:
...
def build_main_text(uid):
    ...
    for h in habits:
        ...
        text += f"{medal} *{h['name']}* — {total_done} ✅\n"
    return text
