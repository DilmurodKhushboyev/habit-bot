...
def scheduler_loop():
    ...
    schedule.every().day.at("04:01").do(
        lambda: send_monthly_reports() if datetime.now().day == 1 else None
    ).tag("monthly_report")
    # Har yilning 1-yanvari 09:00 (UTC+5 = 04:00 UTC) da yillik hisobot
    schedule.every().day.at("04:02").do(
        lambda: send_yearly_reports() if (datetime.now().month == 1 and datetime.now().day == 1) else None
    ).tag("yearly_report")
    while True:
...
def handle_text(msg):
    ...
    if state in ("admin_bc_private", "admin_bc_groups", "admin_bc_all") and uid == ADMIN_ID:
        ...
        # Media group (album) bo'lsa — barcha xabarlarni yig'ish uchun 1.5 sek kutamiz
        if media_grp_id:
            if not hasattr(bot, '_bc_album_buffer'):
                bot._bc_album_buffer = {}
            if media_grp_id not in bot._bc_album_buffer:
                bot._bc_album_buffer[media_grp_id] = []
            bot._bc_album_buffer[media_grp_id].append(msg.message_id)
            # Agar allaqachon ishlov berilayotgan bo'lsa — qaytamiz
            if len(bot._bc_album_buffer[media_grp_id]) > 1:
                return
            # Birinchi xabar — 1.5 sek kutib qolganlarni yig'amiz
            time.sleep(1.5)
            msg_ids = bot._bc_album_buffer.pop(media_grp_id, [msg.message_id])
        else:
        ...
        for target_uid_str in users:
            ...
            try:
                if len(msg_ids) > 1:
                    # Album — copy_messages (forward yozuvsiz, formatlash saqlanadi)
                    bot.copy_messages(
