...
def main_menu_dict(uid=None, page=1):
    ...
    if uid:
        ...
        if groups:
            rows.append([{"text": "━━━ 👥 Guruhlar ━━━", "callback_data": "noop"}])
            for g_info in groups:
                ...
                for h in g_habits:
                    ...
                    if i_done:
                        label = f"☑️ {g_info['name']}: {h_name}"
                        rows.append([{"text": label, "callback_data": f"group_done_{g_info['id']}_{h_id}"}])
                    else:
                        label = f"✅ {g_info['name']}: {h_name}"
                        rows.append([{"text": label, "callback_data": f"group_done_{g_info['id']}_{h_id}", "style": "success"}])
    rows.append([
                ...
                for mid in g_data.get("members", []):
                    ...
                    if isinstance(v, dict):
                        ...
                        if bool(v) and h_id == "main":
                            done_count_g += 1
                status = "☑️" if i_done else "✅"
                text  += f"{status} *{g_info['name']}: {h_name}*"
                text  += f"  👥 {done_count_g}/{members_count}\n"
        text += "━" * 16
    return text
...
def _send_group_view(uid, u, g, g_id):
    ...
    # Har bir odat uchun "Bajardim" tugmasi
    for h in habits:
        h_id   = h["id"]
        h_name = h["name"]
    ...
    # Admin tugmasi: odat qo'shish
    if is_admin:
        kb_g.add(cBtn("➕ Odat qo'shish", f"group_habit_add_{g_id}", "primary"))
