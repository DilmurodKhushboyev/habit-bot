...
| `flask_routes_extra.py` | ~890 | `/api/achievements` (har karta uchun `earned_at` sana + `desc` matn — `earned`'ga qarab `desc_done`/`desc_todo` dan tanlanadi, lang asosida tarjima), **`/api/shop`** (15 mahsulot: 14 ball + 1 Stars `gift_box`), **`/api/shop/buy`** / **`/api/shop/sell`** / **`/api/shop/activate`** (hammasi markaziy `db_lock.user_lock(uid)` + `except TimeoutError` → 429 bilan himoyalangan — checkin oqimi bilan AYNI lock registry, ball race sinxron), **`/api/shop/stars_invoice`**, `/api/friends` (`add`/`remove` — mutual friends `two_user_locks(uid, fid)`, `uid==fid` guard), `/api/challenges` (`accept` — `two_user_locks(uid, from_uid)` + lock ichida challenge/user qayta o'qish + atomik DB guard `update_one({status:"pending"})` `modified_count==0` → double-accept yopildi, ball yechishdan OLDIN), `/api/reminder`, `/api/share-card`, webhook. **S1/S2 migratsiyasi (audit #21):** eski `_get_shop_lock` per-user `threading.Lock` registry → `db_lock` ga markazlashtirildi (Qoida #17). Network (`bot.send_message`) lock'dan tashqarida (#18 referral qarori) |
...
**Backend:** `flask_routes_extra.py` shop (buy/sell/activate) markaziy `db_lock.user_lock(uid)` + `try/except TimeoutError` → 429 (3s). **Frontend:** `app-social.js` `_shopActionLock = new Set()` double-tap guard. Yangi endpoint ball/inventory o'zgartirsa — `with user_lock(uid)` bilan o'rang (checkin oqimi bilan bir registry).
...
| `db_lock` | — (faqat `threading`, `contextlib`) | `handlers_text`, `callbacks_shop`, `callbacks_checkin`, `callbacks_checkin_done`, `flask_routes_data`, `flask_routes_extra` (+ kelajakda `scheduler` — S1 migratsiyasi davom etmoqda) |
...
| `flask_routes_extra` | `achievements`, `bot_setup`, `config`, `database`, `db_lock`, `flask_helpers`, `helpers`, `scheduler`, `texts` | `flask_api` |
