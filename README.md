...
> **Shahar (City) gamification:** har odat tasdiqlanganda → shaxsiy isometric 30×30 grid'da bino qurilishi davom etadi, **66 kun = to'liq qurilgan bino** (odat shakllanish ilmiy chegarasi). 10 ta bino turi (stadion/kutubxona/masjid/...), 5 vizual stage (foundation→complete), foydalanuvchi shahar sahifasida turini o'zgartirishi mumkin. Daraxt/gul/avtomobil/bench/fountain — bozordan ball bilan (binolar emas). Kun o'tkazilsa soft regress (-1, 0 da clamp). Construction Insurance premium feature (30 kun progress saqlash). Backend to'liq deploy bo'lgan (`CITY_GRID_SIZE=30` — PHASE C2.1 da 20→30 kengaytirildi, eski user binolari saqlanadi); frontend `app-city.js` PHASE C2.1 yakunlandi — statik 30×30 isometric SVG grid (900 katak, premium oq palitra `#FFFFFF`/`#F5F5F7`, dark gray `#28282C`/`#232327`), erkin diagonal scroll (`touch-action: pan-x pan-y` + `overscroll-behavior: contain`), `position: fixed` konteyner header/bottom-nav orasiga, auto-scroll grid markaziga (`requestAnimationFrame`). Keyingi bosqichlar: C2.2 touch pan, C2.3 zoom, C3 binolar/dekoratsiyalar SVG, C4 API integratsiya
>
...
super_habits/static/
│
├── index.html               ← (~329 qator) HTML layout, splash, modallar, nav, **FAB**
├── style.css                ← (~3284 qator) neumorphism, dark mode, animatsiyalar, **FAB stillari**
...
└── app-city.js              ← (~111 qator) Shahar sahifasi: 30×30 isometric SVG grid (PHASE C2.1 yakunlandi, C2.2+ da pan/zoom/binolar)
```
...
index.html (WebApp entry point)
    ...
    │
    └── app-city.js .............. app-core.js (strings.js — C7 da matn qo'shiladi)
        ↑ loadCity, renderCityGrid, cityIsoX, cityIsoY (PHASE C2.1 — 30×30 isometric grid;
        ↑ C2.2 touch pan, C2.3 zoom, C3 binolar/dekoratsiyalar, C4 API integratsiya)
```
...
| `city_logic.py` | ~400 | **Shahar (City) logikasi** — sof funksiyalar, DB ga to'g'ridan-to'g'ri yozmaydi (chaqiruvchi `save_user` qiladi). `find_empty_slot(udata)` random bo'sh katak, `get_building_stage(progress)` 0-66→0-4 vizual bosqich, `create_building(udata, habit_id, building_type=None, x=None, y=None)` idempotent (bir habit_id bir bino), `update_building_progress(udata, habit_id, delta)` +1/-1 clamp 0..66 + insurance check + **eski user uchun auto-create** (delta>0 bo'lsa), `change_building_type(udata, habit_id, new_type)` progress saqlanadi, `delete_building_for_habit(udata, habit_id)`, `place_decoration(udata, decoration_type, x?, y?)`, `delete_decoration(udata, item_id)`, `move_item(udata, item_id, new_x, new_y)` band katakka False, `activate_insurance(udata, days)`, `_is_insurance_active(city)` |
...
| `callbacks_habits.py` | ~866 | Odat qo'shish, turi tanlash (simple/repeat), repeat count, o'chirish, `toggle_`/`done_`/`skip_`, shield. **City integration:** `confirm_delete_*` → `delete_building_for_habit(u, habit_id)`, simple/repeat fully done → `update_building_progress(u, habit_id, +1)`, simple/repeat fully undo → `update_building_progress(u, habit_id, -1)` (try/except ichida — city xato bo'lsa checkin buzilmaydi) |
...
| `flask_routes_city.py` | ~401 | **City (Shahar) API** — 7 ta endpoint, shop pattern bilan moslashtirilgan (per-user `_get_city_lock` 3s timeout, `add_points_history` har ball o'zgarishida, 3 tilda xato xabarlari `_CITY_ERR` dict). `GET /api/city/<uid>` (to'liq shahar — har bino'ga `stage` (0-4) qo'shilgan, eski user uchun avto-init va save), `POST /api/city/<uid>/move` (item_id ni (x,y) ga ko'chiradi — band katak → 400, grid tashqarisi → invalid_coord), `POST /api/city/<uid>/change_type` (bino turini o'zgartirish — progress saqlanadi, foydalanuvchi shahar sahifasidan bino bossin), `GET /api/city/<uid>/decorations_shop` (dekoratsiya bozori — 5 ta tur, narxlar, foydalanuvchi balli, insurance narx/davomiyligi), `POST /api/city/<uid>/buy_decoration` (atomic: ball tekshiruvi + place_decoration + ball yechish + add_points_history — shahar to'la bo'lsa city_full xatosi va ball yechilmaydi), `POST /api/city/<uid>/delete_decoration` (ball qaytarib berilmaydi — shop pattern), `POST /api/city/<uid>/buy_insurance` (Construction Insurance — 200 ball, 30 kun, qayta sotib olish bekor qilinadi). `register_city_routes(app)` orqali `flask_api.py` da ro'yxatdan o'tadi |
...
| `app-city.js` | ~111 | **Shahar sahifasi** (PHASE C2.1 — statik 30×30 isometric grid). `loadCity()` — `loadTab('city')` ichidan chaqiriladi, `#city-content` ga render qiladi. **Konstantalar (§17):** `CITY_GRID_SIZE=30` (backend `config.py` bilan sinxron — Qoida #11), `CITY_TILE_W=80`, `CITY_TILE_H=40` (klassik 2:1 isometric nisbat), `CITY_PADDING=40`. **Helper'lar:** `cityIsoX(x,y)`/`cityIsoY(x,y)` — grid koordinatasini ekran pikseliga aylantirish (formula: `sx=(x-y)*W/2`, `sy=(x+y)*H/2`). **`renderCityGrid(container)`** — 900 ta katak `<polygon>` sifatida SVG'ga chiziladi (har birida `data-x`/`data-y` atributlari, C5 drop target uchun), checkerboard parity `a/b` klasslari, `viewBox` matematikasi romb cho'qqilarini hisobga oladi (`fullW=2560`, `fullH=1320`, `viewX=-1280`), oxirida `requestAnimationFrame` orqali konteyner avtomatik grid markaziga scroll qilinadi (foydalanuvchi shahar markazini ko'radi, cho'qqilarni emas). **CSS klasslar** (`style.css`): `.city-canvas-wrap` (`position: fixed`, header `safe-top+72px` dan bottom-nav `72px+safe-bottom` gacha, `touch-action: pan-x pan-y` erkin diagonal scroll, `overscroll-behavior: contain` PTR'ni bloklaydi, fon `#F0F0F2`/dark `#1A1A1C`), `.city-canvas` (SVG, `user-select: none`), `.city-tile` (`shape-rendering: crispEdges`, stroke light `#E0E0E5`/dark `#2C2C30`), `.city-tile-a` (oq `#FFFFFF`/dark `#28282C`), `.city-tile-b` (soya `#F5F5F7`/dark `#232327`). `#page-city { padding: 0 }` — `.page` ning 20px padding'i override qilinadi. **Kelajakdagi bosqichlar** (kod ichida komment xarita): C2.2 touch pan (drag bilan kamera siljitish, scroll'ni almashtirish), C2.3 pinch-zoom (yoki tugmalar), C2.4 virtual rendering (agar kerak bo'lsa), C3 binolar/dekoratsiyalar SVG, C4 API integratsiya (`GET /api/city/<uid>`), C5 drag&drop + bino bosish modali (`change_type`), C6 dekoratsiyalar bozor modali (`buy_decoration`, `buy_insurance`), C7 tarjima kengaytirish (10 bino + 5 dekoratsiya nomlari × 3 til), C8 premium CSS polish |
...
**Shahar (city) feature o'zgartirmoqchimisiz?** → **Backend logika** (bino qurish, progress, dekoratsiya): `city_logic.py` (sof funksiyalar). **API**: `flask_routes_city.py`. **Konstantalar** (bino turlari, narxlar, kun chegarasi, `CITY_GRID_SIZE=30`): `config.py` CITY bloki. **Habit checkin bilan ulanish** (yangi checkin joyi qo'shilsa): `update_building_progress(u, habit_id, delta)` chaqirilsin — bot (`callbacks_habits.py`), WebApp (`flask_routes_data.py`), daily reset (`scheduler.py`) bilan sinxron pattern (§27). **Frontend shahar sahifasi:** `app-city.js` (PHASE C2.1 — 30×30 isometric grid; C2.2+ da pan/zoom/binolar), `style.css` (`.city-canvas-wrap`/`.city-tile*` qoidalari va keyingi bosqichlarda bino/dekoratsiya qoidalari), `strings.js` `S('city', '*')` — 3 til. **Sahifa konteyneri:** `index.html` `<div id="page-city">`. **Navigatsiya:** `app-core.js` `loadTab()` ichida `case 'city'` → `loadCity()`. **MUHIM (Qoida #11):** `CITY_GRID_SIZE` frontend (`app-city.js`) va backend (`config.py`) o'rtasida sinxron bo'lishi shart — biri o'zgartirilsa, ikkinchisi ham. Kelajakda API response'dagi `grid_size` field'idan frontend dinamik o'qishi mumkin (hozir hardcode).
...
`strings.js` → `app-core.js` (`<head>` da), keyin `app-pages.js` → `app-stats.js` → `app-profile.js` → `app-habits.js` → `app-social.js` → `app-reminders.js` → `app-city.js` (body oxirida). Tartibni buzish global o'zgaruvchilarning topilmasligiga olib keladi.
