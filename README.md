...
├── handlers_text.py          ← Matn xabarlari, majburiy obuna tekshiruvi, to'lov, broadcast, inline query
...
└── static/                      ← Flask static papka (frontend fayllar)
    ...
    ├── app-stats.js             ← (~860 qator) Statistika (alohida sahifa), chartlar, heatmap, reyting
    ├── app-profile.js           ← (506 qator) Profil, tahrirlash, til, dark mode
    ├── app-habits.js            ← (~380 qator) Odatlar CRUD, icon picker, modal (nav tab dan emas, today swipe orqali chaqiriladi)
...
habit_bot.py (entry point)
    ...
    │   └── callbacks_shop.py
    │
    ├── handlers_text.py ........ database, helpers, bot_setup, menus, groups, scheduler
...
| `texts.py` | ~368 | `LANGS` dict — uz/en/ru (3 til). `btn_ack` — xabarni o'chirish tugmasi. Repeat odat kalitlari: `habit_type_choose`, `ask_repeat_count`, `ask_repeat_first_time`, `ask_repeat_next_time`, `repeat_progress`, `btn_type_simple`, `btn_type_repeat`, `btn_no_time`. Share kalitlari: `weekly_share_caption` (bold caption + bot link), `share_fwd_btn`, `share_del_btn` |
...
| `handlers_text.py` | ~1041 | Barcha matn xabarlari (state machine), **majburiy obuna tekshiruvi** (`handle_text` boshida — roʻyxatdan oʻtgan foydalanuvchi xabar yuborganda `check_subscription()` tekshiriladi, obuna yoʻq boʻlsa `send_sub_required()` koʻrsatiladi, admin broadcast state istisno), repeat odat vaqt soʻrash T() bilan, Telegram Stars toʻlov, broadcast, inline query. Import tuzatilgan: `time`, `schedule`, `user_exists`, `admin_menu`, `load_settings`, `save_settings` |
...
| `flask_routes_data.py` | ~523 | `/api/today` (har bir odatda `days_66_done` va `times` qaytaradi), `/api/checkin` (bajarish logikasi), `/api/stats` (summary da yangi fieldlar: `today_done`, `today_total`, `top_habit_name/icon/pct`, `worst_habit_name/icon/pct`) |
| `flask_routes_extra.py` | ~827 | `/api/achievements`, `/api/shop` (buy da `owned` qaytaradi), `/api/friends`, `/api/challenges`, `/api/reminder` (repeat\_times, reminder\_enabled, repeat qo'llab-quvvatlaydi), `/api/share-card/<uid>` (Canvas PNG qabul qilib Telegram chatga yuboradi — caption `<b>` bold HTML, inline tugmalar: 📤 Ulashish + ✅ Tasdiqlash), `/health`, `/favicon.ico` (204), webhook. **Import tuzatish:** `telebot`, `InlineKeyboardMarkup/Button`, `ACHIEVEMENTS as _ACHIEVEMENTS`, `CAT_LABELS` qo'shildi |

### Frontend (Telegram WebApp)

| Fayl | Qatorlar | Vazifasi |
|------|----------|----------|
| `index.html` | ~278 | HTML tuzilma: **splash screen** (`#splash-screen` — neumorphism loading ekran: haqiqiy bot logo `<img>` (`id="splash-logo"`), orbit ring, bouncing dots, 5s ko'rsatiladi, yashirilganda logo header yuqori chapga uchadi), header, **global PTR indikator** (barcha sahifalar uchun), tablar, sahifalar (page-today, page-stats, page-habits (endi **Statistika** sahifasi — `#stats-detail-content`), page-profile...), modallar, bottom nav (Odatlar / Reyting / **Statistika** / Bozor / Profil — Tahrirlash olib tashlandi), badge popup. `page-stats` da faqat **Reyting + Guruh/Doʻst** sub-tab qoldi (Statistika sub-tab olib tashlandi). Hech qanday JS/CSS yo'q |
| `style.css` | ~1460 | Barcha CSS: `:root` ranglar, dark mode, neumorphism shadow'lar, `.header` da `--tg-safe-top` safe area padding, card/button/modal/toast/nav stillar, animatsiyalar (spin, fadeIn, slideUp, confetti), PTR indikator (yuqori + pastki), nav ball glow pulsatsiya (`ballGlow`), toʻlqin ripple effekti (`navRippleWave`), **checkin-card swipe-to-reveal** (`.checkin-front`, `.checkin-actions-bg`, `.cswipe-btn`, `.cswipe-close`), **3-nuqta dropdown** (`.checkin-dots-btn`, `.checkin-dropdown`, `.checkin-dropdown-close`, `cdropIn` animatsiya), **splash screen** (neumorphism: `.splash-icon-wrap` sh-out shadow + pulse, `.splash-orbit` aylanuvchi dashed circle, `.splash-dot` bouncing dots, `.hide` fade-out, **`.fly`** — matn/orbit yashirish, **`.fly-logo`** — logo header joyiga uchish transition), **Stats Cards Variant C** (`.sc-card` min-height 140px, `.sc-header`, `.sc-val`, `.sc-lbl`, `.sc-badge-icon`, `.sc-chart`, `.sc-mini-bars`/`.sc-mini-bar`/`.sc-done`, `.sc-ring-fill`, `.sc-foot` 10px/`.sc-foot-trunc`, animatsiyalar: `scFadeUp`, `scBarPop`, `scRingDraw`), **`.bar-fill` dark mode fix** (`var(--sh-sm)` hardcoded rang oʻrniga), **Heatmap GitHub-style** (`.heatmap-outer`, `.heatmap-day-labels`, 7 qatorli grid, `.hm-lv1`/`.hm-lv2`/`.hm-lv3` rang darajalari, `.today-cell`), **Trend card** (`.trend-card`, `.trend-header`, `.trend-diff-pill`, `.trend-compare`, `.trend-compare-bar`, `.trend-msg`, `.trend-spark-title`) |
| `strings.js` | ~930 | `STRINGS` obyekt (UZ/RU/EN tarjimalar), `S(key, sub)` funksiya, `selectedLang`, `currentLang` o'zgaruvchilari. `days_left` — "Odat shakllanishiga {n} kun qoldi" (3 tilda). `habits.edit_btn`/`habits.delete_btn` — swipe va dropdown tugmalari matni (3 tilda). `nav.habits` — **Statistika** (UZ) / **Статистика** (RU) / **Statistics** (EN). **Yangi stats kalitlari**: `today_momentum` ("Bugungi natija"), `left_today`, `all_done`, `worst_habit`, `worst_warn`, `top_habit`, `no_habits`, `best_week`, `best_week_vs`, `record_label`, `hm_week_days` (hafta kun nomlari), `hm_not_done`/`hm_partial`/`hm_full` (heatmap legend), `trend_vs`, `streak_suffix` (3 tilda) |
| `app-core.js` | 396 | Telegram WebApp init (`tg.ready()`, `tg.expand()`, `tg.requestFullscreen()`, `tg.disableVerticalSwipes()`, `applySafeArea()` — CSS `--tg-safe-top` variable), `user`/`userId`/`API` konstantalari (**`userId` va `initData` — `let` bilan e'lon qilingan**, Telegram kech javob bersa yangilanadi), `data`/`loaded` state, `switchTab()`/`goBack()`/`loadTab()` (profil, bozor va **habits (statistika)** har safar qayta yuklanadi, **`_tabLoading` lock** — tez-tez bosishdan himoya: tab yuklanayotganda yangi switchTab chaqiruvlari e'tiborsiz qoldiriladi, `loadTab` da `try/finally` bilan lock ochiladi; `habits` tab endi `loadStatsPage()` chaqiradi), `apiFetch()` (20s timeout), `showPremiumPage()`/`renderPremium()`/`buyPremium()`, `ringHTML()`/`jonRingHTML()` SVG generatorlar, `spawnNavRipple()` toʻlqin effekti |
...
| `app-stats.js` | ~930 | **Statistika**: `loadStatsPage()` (alohida Statistika sahifasi uchun — `#stats-detail-content` ga render), `renderStats()` (**yangi 4 karta — Variant C Mini Charts**: 1-Bugungi natija mini barlar, 2-Streak sparkline (weekly % dan, hafta kun labellari bilan, viewBox 120x42), 3-Eng zaif odat donut (**72x72**, radius 28, font 15px), 4-Eng barqaror odat donut (72x72); `donutSVG()` helper; SVG icon helperlar; **bar chart** (min 8px balandlik, 0% uchun `·` marker, dark mode safe box-shadow), **30-kun area chart** (bezier, dinamik text-anchor — chekka overflow himoyasi), **haftalik trend** (`.trend-card` CSS class, pill diff badge, 10px barlar — 30-kun sparkline olib tashlandi chunki yuqoridagi area chart bilan dublikat edi), **heatmap GitHub-style** (7 qator hafta kunlari, `monthly` dan 4 daraja rang: lv1/lv2/lv3, padding cells), har bir odat statistikasi (`streak` tarjima qilingan), `toggleHabitStats()`, `generateShareCard()` (Canvas PNG — **1080x1920 Story format (9:16)**, sahifadagi barcha natijalarni kompakt chizadi: bugungi natija, streak + rekord, eng zaif/barqaror odat donutlari, trend, haftalik bar chart, 30-kun area chart, heatmap; canvas oxirida trim qilinadi). `loadStats()` endi faqat Reyting sahifasi uchun. **Reyting**: `loadRating()`, `renderRating()` (podium, medals, sort/period), `userAvatarHTML()`, `setRatSort()`/`setRatPeriod()` |
...
| `app-social.js` | ~1260 | **Guruhlar**: `loadGroups()`, `renderGroups()`, `saveGroup()`, `deleteGroup()`, `groupCheckin()`, `setGroupGoal()`. **Do'stlar**: `loadFriends()`, `renderFriends()`, `searchFriends()`, `addFriend()`, `removeFriend()`. **Shop**: `loadShop()`, `renderShop()`, `buyItem()` (sotib olish/sotishda `data.profile.points` va `loaded.profile` yangilanadi), `sellItem()`, `activateItem()`. **Sub-tab boshqaruvi**: `setStatSub()` (faqat `rating`/`friends` — `stats` olib tashlandi, default `'rating'`), `setGroupSub()`. **Challenge**: `openChallenge()`, `sendChallenge()`, `acceptChallenge()`, `rejectChallenge()`. **Onboarding**: `maybeShowOnboard()`, `obMarkDone()`, `renderOnboard()`. **Init**: `window.onload` (**splash**: subtitle tilga moslash UZ/RU/EN, `hideSplash()` 3s kafolat timer — avval `.fly` class bilan matn/orbit yashiriladi, keyin JS bilan `getBoundingClientRect()` orqali logo header `.logo-icon` joyiga `translate+scale` transition bilan uchadi, 550ms dan keyin `.hide` qoʻshiladi; **userId kech aniqlansa** — `location.reload()` oʻrniga `userId`/`initData` oʻzgaruvchilari yangilanib `loadToday()` chaqiriladi, cheksiz reload loop bartaraf etilgan), `visibilitychange`. **Ovoz**: `playHabitSound()` (done=celebration chime 4-nota marimba+shimmer, undo=soft whomp-down), `playProgressSound(step, total)` (repeat odat oraliq progress — ko'tariluvchi pop). **PTR**: pull-to-refresh universal touch handler — yuqoridan va pastdan (barcha asosiy sahifalar uchun; ichki sahifalar — achievements/reminders/premium — bundan mustasno) |
...
6. **Frontend cache-busting**: barcha `<script src>` va `<link href>` larda `?v=422` query string bor — yangilanganda versiyani oshiring
...
8. **Frontend `window.onload`**: `app-social.js` da — splash subtitle tilga moslash, 3s `hideSplash()` kafolat timer, userId kech aniqlansa reload qilmasdan `userId`/`initData` yangilanib `loadToday()` chaqiriladi

---

## 🔴 TODO: Bahrain → Frankfurt ma'lumot ko'chirish (2026-04-02)

**Vaziyat:** MongoDB Atlas klaster Bahrain (me-south-1) regionida texnik nosozlik tufayli ishlamay qoldi. Yangi Free klaster Frankfurt (eu-central-1) da yaratildi (SuperHabits-EU project).

**Eski klaster:** Project 0 → Cluster0 → `cluster0.i0jux9m.mongodb.net` (Bahrain) — ma'lumotlar saqlanmoqda, server hozircha down
**Yangi klaster:** SuperHabits-EU → Cluster0 → `cluster0.nfqvjlp.mongodb.net` (Frankfurt) — hozir aktiv, bo'sh baza

**Bahrain tiklanganda qilish kerak:**
1. Atlas → Project 0 → Cluster0 (Bahrain) ishlayaptimi tekshirish
2. `mongodump` bilan eski bazadan eksport qilish: `mongodump --uri="mongodb+srv://habitbot:Habit2026@cluster0.i0jux9m.mongodb.net" --db=habit_bot --out=./backup`
3. `mongorestore` bilan yangi bazaga import: `mongorestore --uri="mongodb+srv://habitbot:Habit2026@cluster0.nfqvjlp.mongodb.net" --db=habit_bot --drop ./backup/habit_bot`
4. Bot avtomatik yangi ma'lumotlar bilan ishlaydi (restart kerak emas)
5. Muvaffaqiyatli bo'lgach — eski Bahrain klasterni o'chirish mumkin (Project 0 → Cluster0 → Terminate)
6. Shu eslatmani README dan o'chirish

**⚠️ Muhim:** `--drop` flagi yangi bazadagi mavjud collectionlarni o'chirib, eskisini qo'yadi. Agar foydalanuvchilar yangi bazada odat yaratgan bo'lsa — avval merge skript yozish kerak!
