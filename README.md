...
23. **Inventory "Trofey ko'rgazmasi" (v458)**: Reyting va profil sahifalarida eski `🎒 N` banda o'rniga **eng qimmat top-3 emoji + qolgani `+N`** ko'rsatiladi (masalan `👑🏎️⭐ +2`). **Backend** (`flask_routes_core.py`): `from config import ... SHOP_PRICES` import qilingan, `/api/rating` va `/api/profile` endpoint'laridagi `items_list` elementlariga `"price": SHOP_PRICES.get(iid, 0)` maydoni qo'shilgan (jami 16 joy — 8 rating + 8 profile). Noma'lum id lar uchun `price=0` fallback. **Frontend**: `app-stats.js` `renderRating()` scope ichida `_INV_BADGE_EMOJI` mapping va `_invBadgeDisplay(u)` helper — `items_list` ni `price` bo'yicha kamayuvchi sort qilib top-3 emoji oladi, qolgani `+N` qo'shiladi. 3 joyda ishlatiladi (podium, 4-10 rows, myRow) — font-size 8→10px podiumda, 9→11px rows/myRow da (emoji'lar 8px da juda mayda edi), `letter-spacing:-0.5px` emojilar zich ko'rinishi uchun. `app-profile.js` IIFE ichida inline dublikat (`_badgeEmoji` mapping + sort mantiq) — app-stats va app-profile ikki fayl, lekin hozircha markaziy joyga ko'tarish YO'Q (kelajakda uchinchi foydalanuvchi chiqsa `app-core.js` ga ko'taring). Profil bandasida 🎒 emojisi olib tashlandi va binafsha `#A78BFA` → `var(--accent)` ga (NASA rang intizomi — 19-qoida). **Bosilganda** — eski `openUserInventoryByKey()` modal o'zgarmay ochiladi (modal ichi tegilmagan). **Sabab**: foydalanuvchilar nodir buyumlar (`badge_secret` 600 ball) olishsa ham quruq `🎒 3` bandida maqtana olmasdi — endi emojilar passiv maqtanish sifatida ko'rinadi. **Kelajak bosqichlari**: (2) modal ichini galereyaga aylantirish (katta kartalar, "jami qiymat"), (3) "Kolleksioner" mikro-badge top-kolleksionerlar uchun — hozircha faqat bosqich 1 bajarildi.
