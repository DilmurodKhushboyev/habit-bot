...
| `app-reminders.js` | ~337 | **Bir martalik eslatmalar (Today sahifa bo'limi — odat kartalardan KEYIN ko'rsatiladi):** `loadReminderCards` (cache `_cachedReminders`, `loadToday` parallel chaqiradi — `Promise.all`), `renderReminderSections` (pending+sent filter, Bugun/Keyin ajratish `remind_at` bo'yicha — **"Keyingi eslatmalar" boʻlimi default YOPIQ** collapsible chevron + counter pill `var(--accent)` sarlavha yonida; `_toggleUpcomingRems()` toggle haptic feedback bilan), `_renderRemCard` (neumorphic mini karta: qo'ng'iroq SVG icon + matn + vaqt + ✅/🗑 tugmalar). **Modal:** `openReminderModal` (Tez/To'liq toggle — bugun/ertaga tugmalari + sana input), `saveReminder` (POST `/api/reminders/<uid>`, alert fallback). **Actions:** `markReminderDone` → PATCH `/done` (+2 ball, fade-out 400ms → `loadToday` yangilash), `deleteReminder` → DELETE. Helper: `_remFetch` (apiFetch'dan farqli — backend error body'ni `body.error` orqali o'qiydi), `_formatRemTime`, `_escRemHtml`. **CSS klass nomlari — `rem1-` prefiks** (mavjud `.rem-card` profile kartalari bilan konflikt oldini olish uchun, §11): `.rem1-card`, `.rem1-card-icon`, `.rem1-modal-*`, `.rem1-mode-btn`, `.rem1-day-btn`, `.rem1-section-title`, `.rem1-section-title-collapsible` (chevron rotation), `.rem1-section-count` (yashil pill counter), `.rem1-upcoming-list` (collapsed/expanded — `overflow: visible` ochiq holatda, neumorphism soyalar kesilmasligi uchun). **Strings:** `today.add_reminder` (`＋ Eslatma yaratish`)/`rem_today_section`/`rem_upcoming_section`/`rem_done_btn`/`rem_del_btn`, `rem_modal.*` (20 kalit × 3 til). **Tugmalar:** `.today-add-btn` va `.today-add-rem-btn` combined selector (bir xil stil — oq fon, yashil matn, `＋` belgisi) |
