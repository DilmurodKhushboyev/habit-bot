...
| `app-social.js` | ~1260 | **Guruhlar**: `loadGroups()`, `renderGroups()`, `saveGroup()`, `deleteGroup()`, `groupCheckin()`, `setGroupGoal()`. **Do'stlar**: `loadFriends()`, `renderFriends()`, `searchFriends()`, `addFriend()`, `removeFriend()`. **Shop**: `loadShop()`, `renderShop()`, `buyItem()` (sotib olish/sotishda `data.profile.points` va `loaded.profile` yangilanadi), `sellItem()`, `activateItem()`. **Challenge**: `openChallenge()`, `sendChallenge()`, `acceptChallenge()`, `rejectChallenge()`. **Onboarding**: `maybeShowOnboard()`, `obMarkDone()`, `renderOnboard()`. **Init**: `window.onload` (**splash**: subtitle tilga moslash UZ/RU/EN, `hideSplash()` 5s timer — avval `.fly` class bilan matn/orbit yashiriladi, keyin JS bilan `getBoundingClientRect()` orqali logo header `.logo-icon` joyiga `translate+scale` transition bilan uchadi, 550ms dan keyin `.hide` qoʻshiladi, `loadToday()` parallal), `visibilitychange`. **Ovoz**: `playHabitSound()` (done=celebration chime 4-nota marimba+shimmer, undo=soft whomp-down), `playProgressSound(step, total)` (repeat odat oraliq progress — ko'tariluvchi pop). **PTR**: pull-to-refresh universal touch handler — yuqoridan va pastdan (barcha asosiy sahifalar uchun; ichki sahifalar — achievements/reminders/premium — bundan mustasno) |
