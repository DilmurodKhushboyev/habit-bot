...
| `callbacks_habits.py` | ~796 | `toggle_`/`done_`/`skip_` checkin handlerlari, odat o'chirish, shield. **Eslatma:** bot orqali odat qo'shish wizard'i (`menu_add`/`habit_type_*`/`habit_no_time`/`repeat_*`) audit #3 da o'chirildi — odat qo'shishning yagona joyi WebApp `api_habits_add`. **Ball bonus (audit #5):** 6 ta ball hisoblash joyi (`toggle_` repeat done/undo, `toggle_` simple done/undo, `done_` repeat done, `done_` simple done) — `apply_item_bonuses(u, _base)` (badge/car foiz) + `apply_pet_dog_bonus(u, today, is_undo)` (pet_dog kunlik) chaqiradi, `points_logic.py` dan import; WebApp `api_checkin` bilan to'liq sinxron (Qoida #10). Undo joylarida mavjud `_still_done` tekshiruvi pet_dog qaytarish uchun ham ishlatiladi. **City integration:** `confirm_delete_*` → `delete_building_for_habit(u, habit_id)`, simple/repeat fully done → `update_building_progress(u, habit_id, +1)`, simple/repeat fully undo → `update_building_progress(u, habit_id, -1)` (try/except ichida — city xato bo'lsa checkin buzilmaydi) |
