...
└── static/                      ← Flask static papka (frontend fayllar)
    ...
    ├── style.css                ← CSS (1771 qator) — neumorphism, dark mode, animatsiyalar, shop stillar, profil chiplar
...
| `app-profile.js` | 549 | **Profil**: `loadProfile()`, `renderProfile()` (avatar: `d.photo_url || user.photo_url` — DB dagi rasm birinchi oʻrinda, stats grid, jon bar, **achievements progress bar** (`achPct`/`achColor` bilan), referral, sozlamalar — ~~premiumBadge (Free/Premium yozuvi)~~ **VAQTINCHA OʻCHIRILGAN**). Inventory chiplar CSS classlarga chiqarilgan: `.profile-chips`, `.profile-chip`, `.profile-chip-label`, `.profile-chip-accent`. Bar qatorlari: `.profile-bar-row`, `.profile-bar-label`, `.profile-bar-value`. Hardcoded matnlar tuzatilgan: "Avto" → `S('profile','avto')`, "JON" → `S('profile','jon_label')`. **Tahrirlash**: `openEditProfile()`, `saveEditProfile()`, `previewEpPhoto()`. **Sozlama**: `updateNavLabels()`, `setLang()`, `openLangModal()`/`saveLang()`, `saveDarkMode()`, `saveEveningNotify()`, `updateToggleVisual()`. **YANGI**: `copyRefLink(link)` — clipboard API + `_fallbackCopyRef()` fallback, toast `S('msg','copy_link')` |
