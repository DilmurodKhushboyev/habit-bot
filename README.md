...
| `style.css` | ~2170 | `:root` ranglar (NASA yashil `#4CAF7D`), dark mode, **neumorphism 2.0** shadow'lar (`--sh-out/in/press/sm/btn`), safe area padding, card/button/modal/toast/nav, animatsiyalar (spin, fadeIn, slideUp, confetti, ripple, glow, ballGlow). **Checkin kartochka:** 44×44 neumorphic inset icon, swipe-to-reveal (`.checkin-front`/`.checkin-actions-bg`, subpixel `bottom:1px` fix), 3-nuqta dropdown, pending tugmada yashil ✓ (`:empty::before`) + nozik halqa + aylanuvchi glow SVG ring, `done` karta "faded" `opacity: 0.45` + yashil gradient tugma (Streaks/Habitify pattern). **Profile:** `.profile-chips`/`.profile-chip` (inventory banda + press effekti), `.profile-bar-row` (JON/achievements). **Reyting:** `.inv-badge-clickable` (avatar ustida yashil zar `.avatar-snow-*`). **Bozor:** `.shop-*` (balance, cats, item, buy/sell btn, info-btn, modal-overlay/box), `.shop-cat-btn` aktiv holatda `--sh-in` inset shadow (40px min-height, scale 0.96 press). **Today:** `.today-add-btn` (neumorphic "+ Odat yaratish"). **Onboarding:** `#onboard-modal` (raised steps `--sh-out`, done → `--sh-in`; `.ob-arrow::before` barmoq `👆` `rotate(-15deg)` + `obTapBounce`; `.ob-arrow::after` yashil ripple `obTapRipple` barmoq uchidan tarqaladi; `.done .ob-arrow` yashiriladi). **Celebration:** `all-done-banner` (avto-kollaps 3s), `hero-party-badge` (SVG 🎉), konfetti (42 ta zarracha 3 darajali yashil) |
...
| `app-pages.js` | ~730 | **Bugun:** `loadToday`, `renderToday` (66-kun compact progress bar, swipe-to-reveal, `.today-add-btn`, **pending habit SVG glow ring** — `<svg class="habit-glow-ring">` aylanuvchi yashil nur, matn `.checkin-btn-content` span ichida, **karta meta iconlari** — soat va olov SVG neumorphik kulrang gradient `#A8ADB5 → #8A8F98`, streak = 0 holatida `opacity: 0.5`), `checkin` (done/undo, repeat dots, **konfetti + `_triggerConfetti`**, badge popup, streak milestone, **`wasAllDone` boolean** — konfetti faqat yangi yutuqda, **hero bayram emoji `_partySvg` SVG** yuqori-o'ng burchakda + `bannerCollapse` 3s keyin joyni bo'shatadi, **glow SVG ga tegmaydi** — faqat `.checkin-btn-content` yangilanadi, undo paytida `animation-delay` bilan sinxronlash). **Swipe:** `_initCheckinSwipe` + gesture intent lock (touchmove `passive:false`, gorizontal `locked` bo'lganda `preventDefault`). **Eslatmalar**, **Yutuqlar** |
...
| `app-habits.js` | ~380 | `loadHabits`, `renderHabits`, `openAdd`/`openEdit` (modal ochilgach nom input `scrollIntoView({block:'center'})` — §24), `saveHabit` (`repeat_times` API), `deleteHabit`, `closeModal`. **Dinamik vaqtlar:** `_buildTimeInputs`, `addHabitTime`, `_onRepeatCountChange`, `_sortHabitTimes` (avtomatik chronologik). **Icon picker:** `ICON_CATS` (10 kategoriya, neumorphic 3D dome dizayn), `selectIconCat`, `selectIcon`, `_buildIconGrid`. `openAddFromToday` — today sahifasidan (`_ensureHabitModal` + `_returnToToday` bayroq; onboarding habit step ham shu funksiyani chaqiradi) |
| `app-social.js` | ~1468 | **Guruhlar:** `loadGroups`, `renderGroups`, `saveGroup`, `groupCheckin` (`updateHeaderPts(r.points)` done/undo branch'larda). **Do'stlar:** `loadFriends`, `searchFriends`, `addFriend`, `removeFriend`. **Shop:** `_shopData`/`_shopContentId`/`_shopCat`, `_shopActionLock = new Set()` double-tap guard, `loadShop`, `renderShop` (7 kategoriya: all/protection/bonus/badge/pet/car/gift), `setShopCat` (aktiv tugmani `scrollIntoView({inline:'center'})` bilan markazga tortadi), **`openShopInfo`** (info modal — emoji, nom, vazifa `bozor.info_*`, narx, sotish narxi), **`confirmBuyItem`** → `_doConfirmedBuy` → `_executeBuy` (zanjir: tasdiq → lock → `updateHeaderPts`), `sellItem`/`activateItem`, `closeShopModal` (overlay bosish ham), haptic feedback. **Onboarding:** `OB_TEXT` (uz/ru/en — 2 step: habit/stats), `getObState`/`saveObState` (localStorage `sh_ob_v3`, `_done` bayroq), `maybeShowOnboard` (odat bor → `state.habit = true` avto, habit-modal ochiq bo'lsa ustidan chiqmaydi), `obStepClick` (habit → `openAddFromToday`; stats → `obMarkDone('stats')` darhol), `obMarkDone`, `renderOnboard`, `closeOnboard`. **Challenge**, **Init** (`window.onload`: splash 5s → `loadToday`, visibilitychange). **PTR:** pull-to-refresh (yuqori + pastki, intent lock: `|dx| > |dy|` → bekor, `|dy| > 8` → locked). **Klaviatura:** `visualViewport resize` → aktiv input `scrollIntoView({block:'center'})` (§24). **Ovoz:** `playHabitSound`, `playProgressSound` |
...
### 24. Klaviatura chiqqanda input ko'rinishi (`scrollIntoView` pattern)
WebApp'da input/textarea aktiv bo'lib klaviatura chiqqanda — element ko'rinish maydonida qolishi SHART. Pattern: `scrollIntoView({ behavior: 'smooth', block: 'center' })`. **`block: 'start'` ISHLATMANG** — u elementni ekran yuqorisiga suradi va klaviatura pastdan kelgach input yarim ko'rinadi yoki yuqoriga kirib ketadi. Uchta joyda shu pattern qo'llaniladi: `app-habits.js` `openAdd`/`openEdit` (modal ochilganda nom inputi), `app-social.js` `visualViewport.resize` (klaviatura hodisasi — har qanday aktiv input/textarea uchun umumiy handler). Yangi modal yoki input qo'shilsa — shu pattern'ga ergashing, alohida scroll logikasi yozmang. **Gorizontal scroll** uchun esa `inline: 'center'` ishlatiladi (masalan `setShopCat` tugmasini markazga tortish) — bu boshqa maqsad, `block` parametriga aralashmaydi.
