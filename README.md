...
| `handlers_text.py` | 1030 | Barcha matn xabarlari (state machine), Telegram Stars to'lov, broadcast, inline query. Import tuzatilgan: `time`, `schedule`, `user_exists`, `admin_menu`, `load_settings`, `save_settings` |
...
| `callbacks_settings.py` | 366 | Sozlamalar: til, odat vaqtlari, ism, telefon o'zgartirish. Import tuzatilgan: `time`, `threading`, `schedule`, `ReplyKeyboardMarkup`, `KeyboardButton`, `lang_keyboard` |
| `callbacks_habits.py` | 757 | Odat qo'shish (`menu_add`), o'chirish, `toggle_` (bajarish), `done_`, `skip_`, shield. Import tuzatilgan: `time`, `threading`, `schedule` |
...
| Fayl | Qatorlar | Vazifasi |
|------|----------|----------|
| `groups.py` | 357 | `_save_new_habit()`, `_send_group_view()`, `_build_group_rating()`, `_save_new_group()`. Import tuzatilgan: `time`, `threading` |
| `scheduler.py` | 757 | `send_reminder()`, `daily_reset()`, `schedule_habit()`, `scheduler_loop()`, `send_evening_reminders()`. Import tuzatilgan: `mongo_col`, `groups_col`, `mongo_db` (config dan) |
...
| `flask_routes_core.py` | 555 | `/api/rating`, `/api/profile` (GET + PUT), `/api/habits` CRUD, `/api/groups` CRUD. **Import tuzatish:** `mongo_db`, `mongo_col` (config dan), `ACHIEVEMENTS` qo'shildi |
