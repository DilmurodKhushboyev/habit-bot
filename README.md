...
> **Inventory "Trofey ko'rgazmasi" (v458)** — reyting (podium, 4-10 rows, myRow) va profil sahifalarida eski `🎒 N` kompakt banda o'rniga **eng qimmat top-3 buyum emojilari + qolgani `+N`** (masalan `👑🏎️⭐ +2`). Backend `flask_routes_core.py` `items_list` elementlariga `price` maydoni qo'shildi (`SHOP_PRICES.get(id, 0)`). Frontend `app-stats.js` da `_invBadgeDisplay(u)` helper (renderRating scope ichida), `app-profile.js` da IIFE ichida inline — ikkala faylda `_badgeEmoji` mapping dublikat, kelajakda uchinchi foydalanuvchi chiqsa `app-core.js` ga ko'tarish mumkin. Bosilganda eski `openUserInventoryByKey()` modal o'zgarmay ochiladi. **Sabab**: foydalanuvchilar 5000+ ball sarflab nodir buyum olishsa ham quruq `🎒 3` bandida maqtana olmasdi — endi emojilar darhol ko'rinadi (passiv maqtanish).
> **Header ball sinxron yangilash (v459)** — `app-core.js` da `updateHeaderPts(points)` markaziy helper: DOM (`#header-pts`) + global state (`data.today.points`, `data.profile.points`) bir vaqtda sinxron yangilanadi. **BUG FIX**: avval bozor `_executeBuy()` va `sellItem()` da header ball yangilanmasdi — WebApp qayta ochilgandagina to'g'rilanardi. Endi 6 joyda helper chaqiriladi: `app-pages.js` (loadToday, checkin), `app-social.js` (groupCheckin done/undo, _executeBuy, sellItem). **Kelajakda**: yangi endpoint ball o'zgartirganda faqat `updateHeaderPts(r.points)` chaqirish kifoya, inline kod yozmaslik.
> **Swipe + PTR gesture lock (v460-461)** — barcha touch gesturelar uchun **intent detection** o'rnatildi: foydalanuvchi qaysi yo'nalishda harakat boshlasa, o'sha yo'nalish "qulflanadi", ikkinchisi bloklanadi. **(v460) Swipe**: `app-pages.js` `_initCheckinSwipe()` touchmove listener `{passive: true}` → `{passive: false}` ga o'tkazildi, gorizontal `locked` bo'lganda `e.preventDefault()` chaqiriladi (`e.cancelable` tekshiruvi bilan) — avval odatni chapga surayotganda ekran vertikal scroll ham bo'lardi. **(v461) PTR**: `app-social.js` ikkala PTR blokiga (yuqori `1328-1388`, pastki `1390-1454`) `startX` va `locked` qo'shildi — avval PTR faqat Y o'qini tekshirardi, foydalanuvchi diagonal harakat qilsa yoki gorizontal siljishda biroz pastga egsa ham PTR indikatori chiqib ketardi. Endi `touchmove` da dastlabki 8px da yo'nalish aniqlanadi: `|dx| > |dy| && |dx| > 8` → PTR `pulling = false` (bekor), `|dy| > 8` → `locked = true` (PTR davom etadi). **Xavfsiz yondashuv**: PTR'da `preventDefault()` **ishlatilmadi** (listener'lar `{passive: true}` qoldi) — chunki PTR `document` ga biriktirilgan, `passive: false` butun sahifa scroll performance'ini tushirishi mumkin edi. Gorizontal swipe vaqtida vertikal scroll'ni swipe'ning o'zi (`.checkin-front` ustidagi `preventDefault`) bloklaydi — PTR faqat indikator chiqmasligini ta'minlaydi. **Ta'sir doirasi**: swipe `.checkin-front` ustida, PTR `document` ustida — turli target'lar, konflikt yo'q. **Kelajakda** yangi gesture qo'shilsa (long-press, double-tap, horizontal swipe boshqa sahifada) — shu pattern'ni ishlating: `touchstart` da `startX`/`startY` yozib olish → `touchmove` da dastlabki 8px da yo'nalish aniqlash → g'olib yo'nalishni `locked` bilan belgilash, ikkinchisini bekor qilish. **Revert v461**: ikkala PTR blokidan `startX`, `locked`, `dx`, `Math.abs(dx) > 8` tekshiruvi o'chiriladi. **Revert v460**: `app-pages.js` qator 681 `{passive: false}` → `{passive: true}`, qator 675-676 o'chiriladi.
> **Inventory badge top-3 → top-1 (v462)** — reyting va profil sahifalaridagi trofey bandasida endi **faqat eng qimmat 1 ta buyum emojisi + qolgani `+N`** ko'rinadi (avval top-3 edi: `👑🏎️💎 +6` → hozir: `👑 +8`). `app-stats.js` `_invBadgeDisplay(u)` va `app-profile.js` IIFE ichidagi `.slice(0, 3)` → `.slice(0, 1)` ga o'zgartirildi (2 fayl, 2 qator + komment yangilanishi). Sort mantiqi (`price` kamayuvchi) va `rest = sorted.length - top.length` o'zgarmadi — avtomatik to'g'ri hisoblanadi. **Sabab**: 3 ta emoji ko'p joy egallardi, qatorlar torlashtirar va jarangdor/mayda ko'rinardi — 1 ta eng qimmat emoji aniq "trofey" sifatida chiqadi, qolgan soni `+N` da aniq o'qiladi (minimalist NASA intizomi bilan mos). Backend, modal, CSS tegilmagan. **Revert**: 2 faylda `slice(0, 1)` → `slice(0, 3)` + `index.html` `v=462` → `v=461`.
...
23. **Inventory "Trofey ko'rgazmasi" (v458, v462 da top-3 → top-1)**: Reyting va profil sahifalarida eski `🎒 N` banda o'rniga **eng qimmat top-1 emoji + qolgani `+N`** ko'rsatiladi (masalan `👑 +8`). **v458 da top-3 edi** (`👑🏎️⭐ +2`), **v462 da top-1 ga qisqartirildi** — 3 ta emoji zich joyda jarangdor/mayda ko'rinardi, 1 ta eng qimmat trofey aniq ajraladi. **Backend** (`flask_routes_core.py`): `from config import ... SHOP_PRICES` import qilingan, `/api/rating` va `/api/profile` endpoint'laridagi `items_list` elementlariga `"price": SHOP_PRICES.get(iid, 0)` maydoni qo'shilgan (jami 16 joy — 8 rating + 8 profile). Noma'lum id lar uchun `price=0` fallback. **Frontend**: `app-stats.js` `renderRating()` scope ichida `_INV_BADGE_EMOJI` mapping va `_invBadgeDisplay(u)` helper — `items_list` ni `price` bo'yicha kamayuvchi sort qilib **top-1** emoji oladi, qolgani `+N` qo'shiladi. 3 joyda ishlatiladi (podium, 4-10 rows, myRow) — font-size 8→10px podiumda, 9→11px rows/myRow da (emoji'lar 8px da juda mayda edi), `letter-spacing:-0.5px` emojilar zich ko'rinishi uchun. `app-profile.js` IIFE ichida inline dublikat (`_badgeEmoji` mapping + sort mantiq) — app-stats va app-profile ikki fayl, lekin hozircha markaziy joyga ko'tarish YO'Q (kelajakda uchinchi foydalanuvchi chiqsa `app-core.js` ga ko'taring). Profil bandasida 🎒 emojisi olib tashlandi va binafsha `#A78BFA` → `var(--accent)` ga (NASA rang intizomi — 19-qoida). **Bosilganda** — eski `openUserInventoryByKey()` modal o'zgarmay ochiladi (modal ichi tegilmagan). **Sabab**: foydalanuvchilar nodir buyumlar (`badge_secret` 600 ball) olishsa ham quruq `🎒 3` bandida maqtana olmasdi — endi eng qimmat emoji passiv maqtanish sifatida ko'rinadi. **Kelajak bosqichlari**: (2) modal ichini galereyaga aylantirish (katta kartalar, "jami qiymat"), (3) "Kolleksioner" mikro-badge top-kolleksionerlar uchun — hozircha faqat bosqich 1 bajarildi.

24. **Header ball markazlashtirish (v459 — `updateHeaderPts` helper)**: `app-core.js` `data` o'zgaruvchisidan keyin **markaziy helper**: `updateHeaderPts(points)` — DOM (`#header-pts` → `⭐ N`) + global state (`data.today.points`, `data.profile.points`) bir vaqtda sinxron yangilanadi, `undefined`/`null` qaytib ketadi. **BUG FIX**: avval bozor `_executeBuy()` va `sellItem()` funksiyalari faqat `_shopData.points` va bozor sahifasidagi `#shop-pts-val` animatsiyasini yangilardi — yuqori o'ngdagi `header-pts` eski qiymatda qolardi, WebApp qayta ochilgandagina to'g'rilanardi. Endi 6 joyda helper chaqiriladi: `app-pages.js` `loadToday()` (qator 13), `app-pages.js` `checkin()` (qator 253), `app-social.js` `groupCheckin()` done branch (qator 231), `app-social.js` `groupCheckin()` undo branch (qator 242), `app-social.js` `_executeBuy()` (qator 748) — **YANGI**, `app-social.js` `sellItem()` (qator 783) — **YANGI**. `activateItem()` ball o'zgartirmaydi (faollashtirish), shuning uchun tegilmagan. **Tegilmaganlar**: `app-habits.js` (odat CRUD ball bermaydi), `challenge accept/reject` (backend hozir `points` qaytarmaydi — kelajakda backend yangilansa `updateHeaderPts(r.points)` qo'shing), `addFriend`/do'st qidiruv (boshqa user points, `r.points` != caller points). **Pattern**: yangi endpoint ball o'zgartirganda inline kod yozmang — faqat `updateHeaderPts(r.points)` chaqiring. **Eski pattern olib tashlandi**: avval `groupCheckin` 2 joyda 3 qatorli inline `getElementById('header-pts')` bor edi, `_executeBuy`/`sellItem` da `data.profile.points = r.points` alohida qator edi — hammasi helper ichiga ko'chirildi (DRY). **Revert**: helper o'chiriladi (app-core.js qator 44-56), 6 ta chaqiruv eski inline ga qaytariladi (yoki 2 ta yangi chaqiruvni — `_executeBuy`/`sellItem` — o'chirish kifoya, asosiy bug qaytadi ammo boshqa joylar toza qoladi).
