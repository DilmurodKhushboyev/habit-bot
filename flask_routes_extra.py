...
def register_extra_routes(app):
    ...
    def api_share_card(uid):
        ...
        try:
            ...
            raw_caption = T(uid, "weekly_share_caption")
            parts = raw_caption.split("\n", 1)
            caption_text = "\U0001f4ca <b>" + parts[0] + "</b>"
            if len(parts) > 1:
                caption_text += "\n" + parts[1]
            kb = InlineKeyboardMarkup()
            kb.row(
                InlineKeyboardButton(T(uid, "share_fwd_btn"), url="https://t.me/share/url?url=https%3A%2F%2Ft.me%2FSuper_habits_bot&text=" + T(uid, "weekly_share_caption").replace("\n", "%0A")),
